...
var js_arg = [];
var asal_modal = '';
var link_frm_lap_sp2d = "";
//var link_frm_lap = "";
// HARUS ANGKA
function isNumberKey(e){
  // Allow: backspace, delete, tab, escape, enter and .
    if ($.inArray(e.keyCode, [46, 8, 9, 27, 13, 110, 188,173]) !== -1 ||
...
function toRp_WithDecimal(angka, decimalSeparator, thousandsSeparator, nDecimalDigits){
    var num = parseFloat( angka == '' ? '0,00':angka); //convert to float  
    //default values  
    decimalSeparator = decimalSeparator || ',';  
    thousandsSeparator = thousandsSeparator || '.';  
    nDecimalDigits = nDecimalDigits == null? 2 : nDecimalDigits;  
...
function terbilang_spm(rp, style){
  var hasil = "";

  $.ajax({
      type: "POST",
      url: baseUrl+"index.php?route=footer/getTerbilang",
      data: {rp:rp, stl:style},
      async: false,
      timeout: 10000,
      success: function(response){
        hasil = response;
...
//pemisah - (2018-01-10) dan kalau ada T00:00:00
function getTglINDO2_1(tgl){
  var NmBulan   = ['','Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus',
        'September', 'Oktober', 'November', 'Desember'];

  var split = tgl.split("-");
  
  return split[2].split('T')[0]+" "+NmBulan[split[1]<10 ? split[1].replace('0',''):split[1]]+" "+split[0];
}
...
function CekShowModal(e,modal){
    ...
    switch(modal){
      ...
      break;
      case "lap_akrual":
        if($('#organisasi').val() == 0 || $('#organisasi').val() == '0.0.0.0'){
            $.alertable.alert('Organisasi belum dipilih');
            return false;
        } else {
            showModalLaporan(modal);
...
function showModal(e,modal){
  ...
  switch(modal) {
    case "user":
        if(act == "add"){
          ...
          loadModal = "Ubah Pengguna TA."+Thn_log;
        }
        url_load  = linkLoad;
        break;
     ...
     case "bendahara_pembantu":
        if(act == "add"){
          ...
          loadModal = "Ubah Bendahara Pembantu TA."+Thn_log;
        }
        widthnya = '1200px';
        url_load  = linkLoad;
        break;
    ...
    case "tagihan":
        loadModal = "Setting Pembuatan SPP Terakhir TA."+Thn_log;
        url_load  = linkLoad;
        break;
    ...
    case 'input_spj_bendterima_terima':
        var xbulan = $("#cb_xbulan").val();

        if (linkLoad.split('/')[4]=='pungut') {
          judul = 'Penerimaan';
        }else{
          judul = 'Setor';
        }
        loadModal = "Input "+judul+" Pendapatan";
        url_load  = linkLoad+"?id="+$("#kd_org2").val()+"&bl="+xbulan;
        break;
        
  case 'addlampiranperda':  
        if(act == "add"){
          loadModal = "Tambah Lampiran Perda TA."+Thn_log;
        } else if(act == "edit"){
          loadModal = "Ubah Lampiran Perda TA."+Thn_log;
        }
        url_load  = linkLoad;
        break;

  case 'modal_tambah_akrual':
        loadModal = "Sistem Informasi Keuangan Daerah"
        url_load = linkLoad;
        widthnya = '1200px';
        break;

  case 'cetak_bendterimaskpd':
        loadModal = "Laporan Penerimaan dan Penyetoran";
        url_load  = linkLoad;
        break;

  case 'rekpencairan':  
        if(act == "add"){
          loadModal = "Tambah Rekening Pencairan SKPD TA."+Thn_log;
        } else if(act == "edit"){
          loadModal = "Ubah Rekening Pencairan SKPD TA."+Thn_log;
        }
        url_load = linkLoad;
        break;
  case 'modal_ambil_jurnal':
      loadModal = "Ambil Transaksi TA. "+Thn_log;
      url_load = linkLoad;
      break;
    
  case "kegiatan_kontrak":
    var skpd    = $("#organisasi").val().split("-");
  ...
  case 'rekening_pendapatan':
    loadModal = "REKENING PENDAPATAN";
    url_load  = linkLoad;
    break;

  case 'list_kontrak':
    var skpd    = $("#organisasi").val().split("-");
    var asal = $('#asal_form').val();
    if(asal === undefined){
      url_load  = linkLoad+"?id="+skpd[0];
    }else{
      url_load  = linkLoad+"?id="+skpd[0]+"&asal="+asal;
    }
    loadModal = "DAFTAR KONTRAK";
    break;

  case 'sync_datasipd':
    loadModal = "Upload Data Dari Excel";
    url_load  = linkLoad;
    break;
  }
  document.getElementById("myModalLabel").innerHTML = loadModal;
...
function showModalLaporan(modal){ // joel, 24 jan 2019 =======================================
  ...
  if(link_frm_lap_sp2d){ var linkLoad  = link_frm_lap_sp2d; }
  else { var linkLoad  = link_frm_lap; }
  var act = getUrlVars(linkLoad)["act"];
  ...
  switch(modal) {
    ...
    case "lap_sp2d_barjas": 
        ...
        break;
    case "lap_sp2b": 
        loadModal = "Cetak Laporan Surat Pengesahan Pendapatan dan Belanja  TA. "+Thn_log;
        url_load  = linkLoad;
        break;
    case "lap_sp2d_GU_kepmen": 
        loadModal = "Cetak Laporan Surat Perintah Pencairan Dana - GU TA. "+Thn_log;
        url_load  = linkLoad;
        break;
    case "lap_sp2d_TU_kepmen": 
        loadModal = "Cetak Laporan Surat Perintah Pencairan Dana - TU TA. "+Thn_log;
        url_load  = linkLoad;
        break;
    case "lap_sp2d_nona": 
  ...
  case "lap_sp2d_tolak": 
        ...
        url_load  = linkLoad; 
        break;
  case "lap_sp2d_tu_nihil": 
        loadModal = "Cetak Laporan Surat Perintah Pencairan Dana TU NIHIL TA. "+Thn_log;
        url_load  = linkLoad;
        break;
  case 'laporan_spp':
  ...
  case "lpj_upgu":
      ...
      break;
  case 'lap_lra_bulanan':
      loadModal = "Cetak Laporan Realisasi Anggaran TA. "+Thn_log;
      url_load  = linkLoad;
      break;
  case 'lap_verifikasi':
      loadModal = "Cetak Laporan Verifikasi TA. "+Thn_log;
      url_load  = linkLoad;
      break;
  case 'lap_akrual':
      var skpd = $('#organisasi').val(); 
      // var urai_skpd = $("#organisasi").attr("alt").split(' ').join('+');   
      var urai_skpd = $("#org_tampilkan").val().split(' - ')[1].split(' ').join('+');               
      loadModal = "Cetak Laporan TA. "+Thn_log;
      url_load  = linkLoad+"?skpd="+skpd+"&urai="+urai_skpd;
      break;  
  case "lap_skp_skr": 
        var skpd = "?id="+$('#organisasi').val();
        var jnis = "&js="+$('#lap_jenis').text();
        var masa = "&ms="+$('#lap_masa').text();
        var nomo = "&nm="+$('#lap_nomor').text().toUpperCase().replace(/\//g,'_').replace(' ','+');

        loadModal = "Cetak Laporan Penerbitan SKP / SKR TA. "+Thn_log;
        url_load  = linkLoad+skpd+jnis+masa+nomo;
...
function modal_searching(e,modal){ // joel =============================
  ...
  switch(modal) {
    ...
    case 'list_kegiatan': 
      switch(form_sp2d){
        ...
        case 'spp_laporan':
          ...
          if($("#organisasi").val() == 0){ var skpd = ""; }
          else { var skpd = $("#organisasi").val(); }
        break;
    ...
    case 'edit_advis':
      ...
      loadModal = "Ubah Advis SP2D TA. "+Thn_log;         
    ...
    case 'skpskr_mdl_rek':
      var rowid = $(e).attr('rowid');
      loadModal = "Cari Data Rekening TA. "+Thn_log;         
      url_load  = linkLoad+"?i="+rowid;
      break;
    ...
    case 'list_kasda_sp2d':
      loadModal = "Cari Transaksi SP2D TA. "+Thn_log;         
      url_load  = linkLoad;         
      break;

    case 'list_transaksi_sp2d':
      loadModal = "Cari Transaksi KASDA SP2D TA. "+Thn_log;         
      url_load  = linkLoad;         
      break;

    case 'lihat_rekening_akrual_skpd':
      var skpd = $('#organisasi').val();
      loadModal = "Lihat Rekening TA. "+Thn_log;
      url_load = linkLoad+"?id="+skpd;
      break;
      
    case 'spd_mdl_rinci':
      var skpd = $("#kd_org2").val();
      var nospd = $('#no_spd').val();
      var bulan = $('#bulan').val();
      var jnsdpa = $('#jnsdpa').val();
      var token = "&tk="+skpd+"|"+nospd+"|"+bulan+"|"+jnsdpa;
      var rowid = $(e).attr('rowid');

      if(skpd == ""){
          $.alertable.alert("Organisasi belum dipilih !"); return false;
      } else {
          loadModal = "Cari Data DPA TA. "+Thn_log;         
          url_load  = linkLoad+"?i="+rowid+token; 
      }  
      break;
...
$(document).ready(function() {
  ...
  // LOAD LOADING MODAL
  var images  = '<div width="100%" style="text-align:center;"><img src="/static/images/30.gif"></div>';
...
function render_modal(header,asal,js_arg){
  ...
  $.ajax({
      ...
      success: function(data){
        ...
        // $(".modal-dialog").css('width', '800px');
        $("#ReportModal .modal-dialog").css('width', '800px');
        $(".cover").hide();
      }
  });
}
...
function showModal_rekening_from_modal(e,modal) {
  var url_load  = "";
  var loadModal = "";
  var linkLoad  = $(e).attr('alt');
  var act       = getUrlVars(linkLoad)["act"];
  ...
  switch(modal) {
    case "input_penerimaan_bendterimaskpd":
        // if(act == "add"){
        //   loadModal = "Tambah Pengguna TA."+Thn_log;
        // } else if(act == "edit"){
        //   loadModal = "Ubah Pengguna TA."+Thn_log;
        // }
        loadModal = "Cari Rekening";
        url_load  = linkLoad;
        break;
  }
  document.getElementById("ReportModalLabel").innerHTML = loadModal;
...
function create_datatables(id, datanya, custom_css = {}, custom_attr=[], scrollY = 200){
  custom_attr.push({'onclick':"onclick_event_datatables(this)"});
  $('#'+id+'').DataTable( {
        destroy:true,
        fixedHeader: true,
        "scrollY": scrollY,   
        ...
        data: datanya,
        'createdRow':  function (row, data, index) {
            $.each(custom_css, function(index) {
              $('td', row).eq(index).css(custom_css[index]);
            });

            for (var i = 0; i < custom_attr.length; i++) {
              $.each(custom_attr[i], function(key, value){
                  $(row).attr(key,value);
              }); 
            }
        },
...
function onclick_event_datatables(e){
    $("#list_tabel tbody tr").removeClass('row_selected');    
    $(e).addClass('row_selected');
}

// ********************************************************************************************
// autocomplate data bendahara dan pihak ketiga
var countries = []; var dtbndhr_phk = [];
var hasilnyak;

function get_dataBendahara(){
  var gabungkannn = "";
  $.ajax({
    type: "POST",
    headers: { "X-CSRFToken": csrf_token },
    url: link_autosugestion,
    data: {},
    dataType: 'text',
    success: function(data){
      hasilnyak = JSON.parse(data)['hasil'];
      for (var i = 0; i < hasilnyak.length; i++) {
        countries.push(hasilnyak[i]['namayangberhak']);
        gabungkannn = hasilnyak[i]['norekbank']+'|'+hasilnyak[i]['bank']+'|'+hasilnyak[i]['npwp']+'|'+hasilnyak[i]['namayangberhak'];
        dtbndhr_phk.push(gabungkannn);
      };
    }
  }); 
}
...
function autocomplete(inp, arr, arrx) {
  var currentFocus;
  inp.addEventListener("input", function(e) {
      ...
      for (i = 0; i < arr.length; i++) {
        if (arr[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
          ...
          b.innerHTML += "<input type='hidden' value='" + arrx[i] + "'>";
          b.addEventListener("click", function(e) {
              inp.value = this.getElementsByTagName("input")[0].value;
              closeAllLists();
              get_call_dtbndhr_phk(this.getElementsByTagName("input")[1].value);
          });
          a.appendChild(b);
        }
      }
  ...
  document.addEventListener("click", function (e) {
      ...
      set_if_tidak_ada_data();
  });

  function set_if_tidak_ada_data(){
    var jenis_sp2d_ = '';

    try {
      jenis_sp2d_ = jenis_sp2d;
    }
    catch(err) {
      jenis_sp2d_ = '';
    }
    
    if (jenis_sp2d_ != 'SP2D_BARJAS' && jenis_sp2d_ != 'SP2D_GU' && jenis_sp2d_ != 'SP2D_TU' && jenis_sp2d_ != 'SP2D_GU_NIHIL'){
      if(countries.length != 0 && searchStringInArray($("#bendahara").val(), countries) < 1){
        $('#norek_bendahara').val('');
        $('#bank_bendahara').val('');
        $('#npwp_bendahara').val('');
        closeAllLists();
...
if (typeof(document.getElementById('bendahara')) != 'undefined' 
  && document.getElementById('bendahara') != null) {
  
  autocomplete(document.getElementById("bendahara"), countries, dtbndhr_phk);
}
...
function get_call_dtbndhr_phk(dataz){
  var cintaku = dataz.split("|");

  $('#norek_bendahara').val(cintaku[0]);
  $('#bank_bendahara').val(cintaku[1]);
  $('#npwp_bendahara').val(cintaku[2]);
}

// ************************************ END ******************************************************

$('#btn_lihat_sumberdana').click(function(){
  render_modal('SUMBERDANA','ambil_sumdan_',[$(this).attr('data-subkeg'), $(this).attr('data-skpd')]);
})
 
// JOEL 11 Juni 2021 - untuk pilih tanggal supaya sesuai dengan tulisan value input
function set_Kalender(target_id,tanggal){
  $("#"+target_id).data('daterangepicker').setStartDate(tanggal); 
  $("#"+target_id).data('daterangepicker').setEndDate(tanggal);
}

function modalCetakDPA(e){ // joel, 24 jan 2019 =======================================
  var linkLoad  = $(e).attr('alt');
  loadModal = "Cetak DPA "+Thn_log;
  url_load  = linkLoad;
...
function generate_nomor_auto(tahun, destination, modul, bulan = null, kodeurusan = null, kodesuburusan = null, kodeorganisasi = null, kodeunit = null, jenis_modul = null){
    if(modul == 'SPP' || modul == 'SPM' || modul == 'SP2D' || modul == 'SPD'){
      if($('#organisasi').val() == '0'){
        $.alertable.alert("Silahkan Pilih Organisasi Terlebih Dahulu."); 
        return false;
      }else{
        $.ajax({
                type: "GET",
                url: "/sipkd/public/generate_nomor_auto/",
                data: {modul : modul, kodeurusan : kodeurusan, kodesuburusan : kodesuburusan, kodeorganisasi : kodeorganisasi, kodeunit : kodeunit, jenis_modul : jenis_modul, bulan : bulan, tahun : tahun},
                async: false,
                dataType: "html",
                timeout: 10000,
                beforeSend: function(){ $(".cover").show(); },
                success: function(response){
                    destination.val(JSON.parse(response)['x_nomor_spd']);
                    $(".cover").hide();
                },
                error: function(response){
                  $(".cover").hide();
                  message_ok('error', 'Terjadi Kesalahan.');    
                }
            });
        return true;
      }
