...
function editJurnal(e){
  ...
  if (organisasi=='') {
    ...
    isSimpancuy = false;
    noref = e;
    showModal($('#btn_edit')['0'],'modal_tambah_akrual');
  }
}
...
function ambiljurnal(e){
  var organisasi = $('#organisasi').val();
  if (organisasi=='' || organisasi==0) {
    $.alertable.alert('Organisasi Belum dipilih'); return false;
  } else {
    showModal(e,'modal_ambil_jurnal');
  }
}

function tarikdatajurnal(e){
  var jenisdt  = $(frm_mdl).find('#jenis_data').val();
  var tglawal  = $(frm_mdl).find("#bulan_dari").val();
  var tglakhir = $(frm_mdl).find("#bulan_sampai").val();
  var tanggal  = cekTanggal(tglawal, tglakhir);

  if (jenisdt=='' || jenisdt==0) {
    $.alertable.alert('Jenis Data Belum dipilih'); return false;
  } else if(!tanggal){
    $.alertable.alert("Tanggal AWAL tidak boleh lebih dari tanggal AKHIR !"); return false;
  } else {
      $.ajax({
        headers: { "X-CSRFToken": csrf_token },
        type: frm_mdl.attr('method'),
        url: frm_mdl.attr('action'),
        data: frm_mdl.serialize(),
        dataType: 'json',
        async: false,
        timeout: 10000,
        success: function(res){ 
          $.alertable.alert(res['pesan']);
