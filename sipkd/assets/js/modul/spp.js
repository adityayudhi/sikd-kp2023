...
function clearFormSPP_PPKD(skpd){
    ...
    if((jenis=='gu') || (jenis=='gu_nihil') || (jenis=='tu_nihil')){
      // $('#organisasi').val('');
      // $('#org_tampilkan').val('');
      $('#no_lpj').val(''); 
      table.clear().draw();
    ...
    }else if(jenis == 'tu'){ // TU tambahan mode kepmen =======================
      $('#kode_unit').val('');
      $('#kode_bidang').val('');
      $('#kode_program').val('');
      $('#kode_kegiatan').val('');
      ...
      $(table_data_afektasi_spp.column(4).footer()).html("0,0");     
      $(table_data_afektasi_spp.column(5).footer()).html("0,0");     
      $(table_data_afektasi_spp.column(6).footer()).html("0,0");     
      $(table_data_afektasi_spp.column(7).footer()).html("0,0");     
      $(table_data_afektasi_spp.column(8).footer()).html("0,0");     
      $(table_data_afektasi_spp.column(9).footer()).html("0,0");     
      $(table_data_afektasi_spp.column(10).footer()).html("0,0");
      $('#total_terbilang').children("span").text('Nol Rupiah');   
      $('#total').children("span").text('0,00');   
      $('#totalspp').val('0'); 
      ...
      if($('#tanggal_pelaksanaan').length > 0){
        $("#tanggal_pelaksanaan").val(DateNowInd());
      } 
      ...
      if($('#organisasi').val() != '0' && asal_form != 'tu_mohon'){
        generate_nomor_auto_gu_tu_ls();
      }
      ...
      if ($("#jenis_pengadaan").attr('alt').toLowerCase() == 'db'){ $("#jenis_pengadaan").val(0);
      } else { $("#jenis_pengadaan").val(1); }
      ...
      $('#bank_perusahaan').val('');
      $('#nomor_kontrak').val('');
      $('#nama_rekening_bank_perusahaan').val('');
      $('#npwp_perusahaan').val('');
      ...
      $.ajax({
          ...
          success: function(data){              
            ...
            if(datanya!=''){                
              ...
              $('#nosptjm_xx').val(datanya[0].nosptjm);
              ...
              $('#no_spp').val(JSON.parse(data)['x_nomor_spp']);
              $('#kunci_spd').html('(DRAFT)');      
              $("#kunci_spd").removeClass('disetujui');        
              $('#bendahara').val('');
              $('#norek_bendahara').val('');
              ...
              var isi_status = 'Uang Persediaan (UP) '+$('#organisasi').attr('alt')+' Tahun '+Thn_log;
              $('#status_keperluan').val(isi_status);
              $("#no_spp").attr('readonly', false); 
...
function cekSimpanData(url){    
  ...
  if($('#organisasi').val() == 0){
      ...
      if(jenis=='up'){
        simpanData();
      }else if(jenis=='ls ppkd' || jenis=='tu'){        
        var val = []; 
        var otorisasi = []; 
        var urai = [];       
        var rekening = ''; 
        ...
        var uraktip;
        var asaldtx = $("#jenis_pengadaan").attr('alt');
        if (asaldtx.toLowerCase() == 'db'){ uraktip = 'swakelola';
        } else { uraktip = 'ls bendahara'; }
        var is_love = $("#jenis_pengadaan option:selected").html().toLowerCase().includes(uraktip);

        if($('#urai_kegiatan').val() == ""){
          $.alertable.alert('Kegiatan Belum dipilih');
        } 
        // else if(kode_sumberdana == ""){
        //   $.alertable.alert("Sumberdana harus diisi terlebih dahulu!")
        // }
        // else if($('#pelaksanaan').val() == ""){
        //   $.alertable.alert('Waktu Pelaksanaan belum diisi');
        // }
        else if(!is_love && $('#nama_perusahaan').val() == ""){
          $.alertable.alert('Nama Perusahaan belum diisi');
        }else if(!is_love && $('#bentuk_perusahaan').val() == 0){
          $.alertable.alert('Bentuk Perusahaan belum dipilih');
        }else if(!is_love && $('#alamat_perusahaan').val() == ""){
          $.alertable.alert('Alamat Perusahaan belum diisi');
        }else if(!is_love && $('#pimp_perusahaan').val() == ""){
          $.alertable.alert('Nama Pimpinan belum diisi');
        }else if(!is_love && $('#npwp_perusahaan').val() == ""){
          $.alertable.alert('NPWP belum diisi');
        }else if(!is_love && $('#bank_perusahaan').val() == ""){
          $.alertable.alert('Nama Bank Perusahaan belum diisi');
        }else if(!is_love && $('#norek_perusahaan').val() == ""){
          $.alertable.alert('No. Rekening Perusahaan belum diisi');
        }else if(!is_love && $('#nomor_kontrak').val() == ""){
          $.alertable.alert('No. Kontrak Perusahaan belum diisi');
        }else if(!is_love && $('#nama_rekening_bank_perusahaan').val() == ""){
          $.alertable.alert('Pemilik Rekening Perusahaan belum diisi');
        }
...
function simpanDraftLPJ(e){  
  ...
  if(chekDraftSPP == false){
    $.alertable.alert("Nomor LPJ Belum ada yang dipilih");
        return false;
  }else{
    $.ajax({
      type: "POST",
...
function ambilAfektasi(from_where = ''){    
  var skpd = $("#organisasi").val();  
  var nospp;
  ...
  var no_lpj = $("#no_lpj").val();
  var item_table_multi = [];
  ...
  if($('#asal_form_modal').length > 0){
    asal_form = $("#asal_form_modal").val();
  }

  if(asal_form == 'tu_mohon'){
    if($('#no_spp_mohon').length > 0){
      nospp = $("#no_spp_mohon").val();
    } else {
      nospp = $("#no_spp").val();
    }
  ...
  if(jenis == 'ls'){    
    if(skpd!='' && kd_bidang!='' && kd_program!='' && kd_kegiatan!=''&& kd_subkegiatan!=''){
      ...
      item_table_multi = tablekegiatan_multi.column(0).data().toArray();
    } 
  ...
  if(skpd==0){
    ...
    $.ajax({
        ...
        success: function(response){  
          if(jenis == 'tu'){
            let kegaitanTU = response.kegiatan_tu
            table_data_afektasi_spp.clear().draw();
            list_spp = response['list_spp'];
            is_hidden = response['hidden'];
            for (var i = 0; i < list_spp.length; i++) {
              is_checked = ''
              if (list_spp[i]['cek'] == 1){
                is_checked = 'checked=checked';
              }

              let split_untuk_kelas = list_spp[i]['koderekening'].split("-")
              class_row = split_untuk_kelas[0].replaceAll('.', '_');
              let kegiatannya = kegaitanTU.find(o => o.kode_kegiatan === split_untuk_kelas[0]).urai_kegiatan

              var temp_table_data_afektasi_spp = table_data_afektasi_spp.row.add([
                        `<td>${list_spp[i]['otorisasi']}</td> `,
                        `<td align="center" ${is_hidden}>
                             <input type="checkbox" name="afektasispp" id="afektasispp_${id_table}" 
                             ${is_checked}
                             value="${list_spp[i]['koderekening']}|${list_spp[i]['sekarang']}|${list_spp[i]['otorisasi']}|${list_spp[i]['uraian']}|${list_spp[i]['lalu']}|${list_spp[i]['batas']}|${list_spp[i]['loloskan']}" class="afektasichk" onclick="handleClick(this,'#input_tbl${id_table}')" />
                         </td>`, 
                         `<td>${list_spp[i]['koderekening']}</td>`, 
                         `<td>${kegiatannya}</td>`,
                         `<td>${list_spp[i]['uraian']}</td>`,
                         `<td align='right'>${list_spp[i]['anggaran']}
                            <input type="hidden" id="anggaran_${id_table}" value="${list_spp[i]['anggaran']}">
                          </td>`,
                          `<td align='right'>${list_spp[i]['batas']}</td>`,
                          `<td align='right' class="lalu${id_table}">${list_spp[i]['lalu']}</td>`,
                          `<td align='right' class="row_sekarang">                        
                            <input class="input-tabel input_sekarang[]" id="input_tbl${id_table}" name="input_sekarang[]" type="text" value="${list_spp[i]['sekarang']}" onkeydown="return isNumberKey(event)" onfocus="EditOnFokus(this.value, 'input_tbl${id_table}',${id_table})" onblur="EditOnBlur(this.value, 'input_tbl${id_table}',${id_table})" style="text-align:right; width:100%;"
                            alt="${list_spp[i]['loloskan']}">
                          </td>`,
                          `<td align='right' id="span_jml${id_table}" class="row_jml">${list_spp[i]['jumlah']}</td>`,
                          `<td align='right' id="sisa_${id_table}" class="row_sis">${list_spp[i]['sisa']}</td>`,
                      ]).draw( false ).node()

                $(temp_table_data_afektasi_spp).addClass(class_row);
                $('td', temp_table_data_afektasi_spp).eq(7).addClass(`lalu${id_table}`);
                $('td', temp_table_data_afektasi_spp).eq(10).attr('id', `sisa_${id_table}`);
                $('td', temp_table_data_afektasi_spp).eq(9).attr('id', `span_jml${id_table}`);
              ...
              table_data_afektasi_spp = $('#dataspp').DataTable( {
                      ...
                      'createdRow':  function (row, data, index) {
                          for (var i = 4;i<=8;i++) {                    
                              ...
                              $('td', row).eq(8).addClass(`row_sekarang`);
                              $('td', row).eq(9).addClass(`row_jml`);
                              $('td', row).eq(10).addClass(`row_sis`);
                          }       
                      },
    ...
    $.ajax({
      ...
      success: function(data){       
        ...
        if(jenis=='gu'||jenis=='gu_nihil'||jenis=='tu_nihil'){
          ...
          $("#sumberdana").val(data['kode_sumberdana']);
          $("#kode_sumberdana").val(data['kode_sumberdana']);
          ...
          $("#bank_perusahaan").val(data['namabankperusahaan']);
          $("#nomor_kontrak").val(data['nokontrak']);
          $("#npwp_perusahaan").val(data['npwpperusahaan']);
          $('#nama_rekening_bank_perusahaan').val(data['pemilikrekeningperusahaan']);
          $("#norek_perusahaan").val(data['norekperusahaan']);
          $("#pelaksanaan").val(data['waktupelaksanaan']);
          $("#tanggal_dpa").val(data['tgldpa']);
          
          if (data['jenispengadaan']==null){
            if ($("#jenis_pengadaan").attr('alt').toLowerCase() == 'db'){ $("#jenis_pengadaan").val(0);
            } else { $("#jenis_pengadaan").val(1); }
          }else{
            $("#jenis_pengadaan").val(data['jenispengadaan']);
          }
          ...
          if(data['kode_sumberdana'] != null){
            $("#sumberdana").val(data['kode_sumberdana'].replace('|',' - '));
            $("#kode_sumberdana").val(data['kode_sumberdana']);
          }

          if(data['tgl_pelaksanaan'] != undefined){
        ...
        if(jenis == 'tu' && $('#asal_form_modal').length > 0){
          $('input[name="aksi"]').val(true);
          $("#kunci_spd").html('(DRAFT)');
          $("#kunci_spd").removeClass('disetujui');
          $("#no_spp").attr('readonly', true);
          $("#no_spp_lama").val('');
...
function ambilSPD(){
  ...
  var kd_bidang = ''; var kd_program = '';
  var kd_kegiatan = ''; var kd_subkegiatan = '';

  if ($("#kode_bidang").length && $("#kode_kegiatan").length) {
      kd_bidang = $("#kode_bidang").val(); 
      kd_program = $("#kode_program").val();
      kd_kegiatan = $("#kode_kegiatan").val();
      kd_subkegiatan = $("#kode_subkegiatan").val();
  } else {
      if($("#kd_kegiatan_xx").length && $("#kd_kegiatan_xx").val() != ""){
        var kodexx = $("#kd_kegiatan_xx").val().split("-")[0].split("."); //1.01.1.2.01.1
        kd_bidang = kodexx[0]+"."+kodexx[1]; 
        kd_program = kodexx[2];
        kd_kegiatan = kodexx[3]+"."+kodexx[4];
        kd_subkegiatan = kodexx[5];
      }
  }
...
function EditOnBlur(e, target, nomer){    
  ...
  var spp_sekarang;
  var jenis = $("#jenis").val();
  var loloskan = $("#"+target).attr("alt");

  // console.log(jenis+" = "+loloskan);
  ...
  var gabung      = valCB[0];
  var gabung2      = valCB[2]+'|'+valCB[3]+'|'+valCB[4]+'|'+valCB[5]+'|'+valCB[6];  
...
function getTotalspp_sekarang(){
    ...
    if(jenis == 'tu'){
      var column = table_data_afektasi_spp.column(idxCol); // idxCol dari index colomn, menyiapkan kolom yg akan diupdate
    }else{
...
function getJumlah(){ // UPDATE JUMLAH TOTAL FOOTER KOLOM JUMLAH ALOKASI DAN SISA ALOKASI
    ...
    if(jenis == 'tu'){
      $(table_data_afektasi_spp.column(9).footer()).html(toRp_WithDecimal(ALTot));
      $(table_data_afektasi_spp.column(10).footer()).html(toRp_WithDecimal(ALSis));
    }else{
...
function PageLoadBKP_UPGU(skpd){
  loadDataBKP_UPGU(skpd);
...
function loadDataBKP_UPGU(skpd){ 
  $.ajax({
    type: "POST",
    headers: { "X-CSRFToken": csrf_token },
...
function pilihJnsLaporan_spp(val){
  if(val == 0 || val == 1 || val == 2|| val == 3){
      $('#col_kegiatan').css('display','none');
    } else if(val == 4){
      $('#col_kegiatan').css('display','');
    }
}
...
function cetakLaporanSPP(){
  ...
  if(skpd == 0){
      ...
      return false;
  } else if(kegiatan == "" && jnsLap == 4){
      $.alertable.alert("Kolom Sub Kegiatan belum dipilih !");
      return false;
  } else {
    $.ajax({
...
function LoadInputBKP_UPGU(){  
  ...
  $.ajax({
    ...
    headers: { "X-CSRFToken": csrf_token },
    data: {skpd:skpd, nobkp:$("#no_bkp").val(), aksi:isSimpan}, // var route from main-mod.js
    async: false,
    dataType: "html",
    timeout: 10000,
    success: function(response){      
...
function clearFormLPJ_UPGU(){
  ...
  enable_disable_btn('1', '0', '0', '0');
}

function enable_disable_btn(mhn, keg, rek, lpj){

  loadPertama('btn_permohonan',mhn);
  loadPertama('btn_kegiatan',keg);
  loadPertama('btn_rekening',rek);
  loadPertama('btn_lpjupgu',lpj);

  if(mhn == '1'){
      $("#permohonan").removeClass('hidden');
...
function addkegiatanBKP_UPGU(){
  ...
  $.ajax({
    ...
    headers: { "X-CSRFToken": csrf_token },
    data: {skpd:skpd, nobkp:$("#no_bkp").val(), kondisi:$("#kondisi").val()},
    async: false,
    dataType: "html",
    timeout: 10000,
    success: function(response){
...
function nextDaftarKegiatan(){
  ...
  if($("#uraian").val() == ""){
      ...
      var frm  = $('#formModal');
    //   $('input[name=daftarkegiatan]:checked').each(function(index){
    //   if (check_sumberdana.length != 0) {
    //     if(jQuery.inArray($(this).attr('data-sumdan'), check_sumberdana) == -1) {
    //       message_ok('error', 'Tidak Boleh Memasukkan Kegiatan Dengan Sumberdana Berbeda Dalam Satu NOLPJ !');
    //       check_sumberdana = $.grep(check_sumberdana, function(value) {
    //                           return value != $(this).attr('data-sumdan');
    //                         });
    //       safetoinsert = false;
    //       return false;
    //     }else{
...
function EditLPJ_UPGU(){
  ...
  if($("#organisasi").val() == 0){
    ...
    if ($(".chk_setuju:checked").attr("locked") == 'Y') {
      $.alertable.alert("LPJ Sudah DiVerfikasi Tidak Diperkenankan Mengedit, Hubungi Bidang Akuntansi !");
      return false;
    }else{
      $('#modalInputLPJUPGU').modal('show');
      LoadInputLPJ_UPGU();
      ...
      $.alertable.confirm('Anda Yakin ingin menyimpan data rincian LPJ UP/GU ?').then(function () {        
            $.ajax({
              ...
              success: function (data) {
                ...
                isSimpan = false;
                $('input[name="aksi"]').val(isSimpan);
                PageLoadLPJ_UPGU($('#organisasi').val());
                // isMdl_Potongan = 'true';
                // $('#modalInputLPJUPGU').modal('hide');
              }
            });
      ...
      $.alertable.confirm('Anda Yakin ingin menyimpan data rincian LPJ TU ?').then(function () {
        $.ajax({
              ...
              success: function (data) {
                ...
                loadDataLPJ_TU(skpd);
                nextRekeningToRinci_TU();
                isSimpan = false;
                $('input[name="aksi"]').val(isSimpan);
                // $('#modalInputLPJ_TU').modal('hide');
              }
            });
        },function(){ 
            message_ok('error', 'Data rincian LPJ TU batal disimpan !');
...
function changeJns_pengadaan(){
  var is_love = $("#jenis_pengadaan option:selected").html();
  var is_aidi = $("#jenis_pengadaan").val(); 
  var arr_lop = ["nomor_kontrak","nama_rekening_bank_perusahaan","nama_perusahaan","bentuk_perusahaan","alamat_perusahaan","pimp_perusahaan",
                "npwp_perusahaan","bank_perusahaan","norek_perusahaan"];
  ...
  if(is_love.toLowerCase().includes(uraktip)){
    for (var i = 0; i < arr_lop.length; i++) { $("#"+arr_lop[i]).prop("disabled",true); };
    $("#baten_kontrak").removeClass("baten");
    $("#baten_perusahaan").removeClass("baten");
    $("#baten_perusahaan").attr("onclick","");

  } else {
    for (var i = 0; i < arr_lop.length; i++) { $("#"+arr_lop[i]).prop("disabled",false); };
    $("#baten_kontrak").addClass("baten");
    $("#baten_perusahaan").addClass("baten");
    $("#baten_perusahaan").attr("onclick","LoadPerusahaan_LSBARJAS(this)");
  }
}

function showModal_spp_lpj(e,modal){ 
  var url_load  = "";
  var loadModal = "";
  ...
  switch(modal) { 
    case 'sp2d_mdl_potongan':
      ...
      var skpd  = $("#organisasi").val();
      var nolpj = $("#no_lpj").val().replace(/\//g,'^');

      loadModal = "Lihat Rekening Potongan TA. "+Thn_log;         
      url_load  = linkLoad+"?i="+rowid+"&b="+encodeURIComponent(nobuk)+"&t="+tglbuk+"&s="+skpd+"&n="+nolpj;
      break;
...
function showModal_spp_lpj_sumberdana(e,modal,number){ 
  var url_load  = "";
  var loadModal = "";
  console.log(number)
  var linkLoad  = $(e).attr('alt');
...
function delete_row_multi(e, ev){
  ev.preventDefault();

  tablekegiatan_multi.row( $(e).parents('tr') ).remove().draw();
  table_data_afektasi_spp.row( `.${$(e).data('ket_kegiatan').replaceAll('.','_')}` ).remove().draw();

  var a = "."+$(e).data('ket_kegiatan').replaceAll('.','_');

  $("."+$(e).data('ket_kegiatan').replaceAll('.','_')).each(function () {                    
    table_data_afektasi_spp.row( '.'+$(e).data('ket_kegiatan').replaceAll('.','_')).remove().draw();          
  }); 
...
// ================================================================================================
// // ----- tambahan permohonan SPJ========= joel 01 Des 22 =======================================
// ================================================================================================

function loadPermohonan_LPJ(){
  if(isSimpan){    
    getNewNo_inLPJ_UPGU();
  }else{
    ambilNoSPD();  
  ...
  $.ajax({
    headers: { "X-CSRFToken": csrf_token },
    type: "GET",
    url: $("#tabel_permohonan").attr("alt"),
    data: {skpd:skpd, aksi:isSimpan}, // var route from main-mod.js
    async: false,
    dataType: "html",
    timeout: 10000,
    success: function(response){      
        enable_disable_btn('1', '0', '0', '0');          
        $('#tabel_permohonan').html(response);        
    }
  });
...
function next_to_kegiatanLPJ(){
  var CEHK = [];
  $(".permohonan_spj:checked").each(function(){ CEHK.push($(this).attr("value")); });

  if(CEHK.length < 1) {
      $.alertable.alert("Anda belum memilih Permohonan SPJ yang diLPJ kan!");
      return false;
  } else {
      setujui_permohonan_SPJ();
  }
}

function setujui_permohonan_SPJ(){  
  var skpd = $("#organisasi").val();
  var urlx = $("#tabel_permohonan").attr('alt');
  var chkMohonSPJ = $(".permohonan_spj").is(":checked");  

  var CEHK = [];
  $(".permohonan_spj:checked").each(function(){ CEHK.push($(this).attr("data-nopspj")); });
   
  if(chkMohonSPJ == false){
    $.alertable.alert("Permohonan SPJ Belum ada yang dipilih."); return false;
  } else {
    $.ajax({
      headers: { "X-CSRFToken": csrf_token },
      type: "POST",
      url: urlx,
      data: {skpd:skpd, dtx:CEHK},
      beforeSend: function() { $(".cover").show(); },
      success: function(hsl){ 
        var mesage = hsl.pesan.split("|")
        message_ok(mesage[0],mesage[1]);
