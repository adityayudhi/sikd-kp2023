...
function cekSKUP(){
  var dt = new Date(getTglAngka($("#tgl_sp2d").val())); 
	...
	$.ajax({
      ...
      success: function(data){
          ...
          if (JSON.parse(data)['hasil'].length>0) {
            ...
            $('#status_keperluan').val('Belanja Operasi. Pembayaran Uang Persediaan (UP) Tahun Anggaran '+dt.getFullYear());
      	  }
...
function ambilUP(ket,nosp2d, kd_org){
	$.ajax({
      ...
      success: function(data){
          ...
          // Ambil - BANK ASAL DAN NOREK - joel,27 Des 21 GI
