	...
	$.ajax({
      ...
      success: function(data){
          ...
          if (JSON.parse(data)['hasil'].length>0) {
            $('#jumlah_sp2d').val(toRp_WithDecimal(hasil['jumlah']));
            $('#jml_sp2d').text('Rp. '+toRp_WithDecimal(hasil['jumlah']));
            $('#sp2d_terbilang').text(terbilang(hasil['jumlah']));
      	  }
...
function ambilUP(ket,nosp2d, kd_org){
	$.ajax({
      ...
      success: function(data){
          ...
          // if (hasil['sumberdana'].split('|').length > 1) {
          //   $('#frm_sumberdana').val(hasil['sumberdana'].split('|')[1]);
          //   $('#frm_sumberdana_x').val(hasil['sumberdana'].split('|')[0]);
          // }
          // else{
          //   $('#frm_sumberdana').val(hasil['sumberdana']);
          //   $('#frm_sumberdana_x').val('');
          // }

          $('#frm_sumberdana').val('[DANA KHUSUS] - Dana Transfer Umum - Dana Alokasi Umum');
          $('#frm_sumberdana_x').val('2.2.1.01.02');

          var myVal = hasil['norekbankasal'];
          $('#bank_asal').val(hasil['bankasal']);
          $('select#norek_bankasal option').each(function(){ this.selected = (this.text.split(" ")[0] == myVal); });
...
function fungsi_modal(ini, asal_modal){
    ...
    }else if(asal_modal=='ambil_sumdan_'){
      $('#frm_sumberdana').val(tglnya);
      $('#frm_sumberdana_x').val(var_nospd);
    }
}
...
function clearForm(){
  ...
  // $('#frm_sumberdana_x').val('');
  if($('#kd_org2').val() != ''){
    generate_nomor_auto(getTahun($('#tgl_sp2d').val()), $('#no_sp2d'), 'SP2D', getBlnToNum($('#tgl_sp2d').val()), $('#kd_org2').val().split('.')[0], $('#kd_org2').val().split('.')[1], $('#kd_org2').val().split('.')[2], $('#kd_org2').val().split('.')[3], jenis_modul)
    }
  ambilbankData();
}
...
function simpan_sp2d(){
  ...
  if ($('#no_sp2d').val()=='') {
    ...
    message_ok('error', 'SUMBERDANA BELUM DIPILIH !');
    ...
    $.ajax({
        ...
        data: {'kd_org':kd_org,
        		...
        		'nosp2d':$('#no_sp2d').val(),
    				...
    				'triwulan':'1',
    				// 'sumberdana':'DAU',
            'sumberdana':$('#frm_sumberdana_x').val()+'|'+$('#frm_sumberdana').val(),
    				'lastupdate':'NOW',
