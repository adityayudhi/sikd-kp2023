...
$(document).ready(function () {
   ...
   $('#table_dasar_spd').DataTable( {
    ...
    $('#tgl_spm').daterangepicker({
          ...
          xx = generate_nomor_auto(start.format('YYYY'), $('#no_spm'), 'SPM', start.format('MM'), $('#kd_org2').val().split('.')[0], $('#kd_org2').val().split('.')[1], $('#kd_org2').val().split('.')[2], $('#kd_org2').val().split('.')[3], jenis_modul);
        });
...
function fungsi_modal(ini, asal){
    ...
    if (asal == 'SPM'){
      ...
      generate_rinci_spm(var_nospm,var_skpd.split('-')[0].split('.')[3]);
      generate_rekening(var_nospm,var_skpd.split('-')[0].split('.')[3]);
    }else if (asal == 'SPM_LS'){   
      $nospm  = row.find('td:nth-child(1)');
      $tgl_spm  = row.find('td:nth-child(2)');
      $skpd = row.find('td:nth-child(3)');
      ...
      generate_rinci_spp(var_nospp, var_skpd.split('-')[0].split('.')[3]);
    }else if (asal == 'SPP_LS'){ 
      $nospp  = row.find('td:nth-child(1)');
      $tgl_spp  = row.find('td:nth-child(2)');
      $skpd = row.find('td:nth-child(3)');
    ...
    $.get(is_skpkd_js(var_skpd2[0]+'.'+var_skpd2[1]+'.'+var_skpd2[2])).done(
        ...
        if (asal == 'SPM'){
          ...
          $('#frm_unit').val(var_skpd.split('-')[0].split('.')[3]); 
          // $('#terbilang').text(' '+terbilang(toAngka(var_up).toString()))
          $('#label_jml_spp_lama').text(toRp_WithDesimal(var_up));
          $('#terbilang').text(' '+terbilang(toAngkaDec(toRp_WithDesimal(var_up).toString())))
          
          generate_dataTable_spm(var_skpd.split('-')[0].split('.')[3]);
...
function generate_dataTable_spm(kodeunit=''){ //load data from SPM (edit data)
...
function generate_dataTable_afektasi_spm(){ //load data from SPP (new data)
...
function generate_rinci_spm(no_spm, kodeunit=''){
  ...
  $.ajax({
      ...
      success: function(data){
        ...
        $('#bendahara').val(bendahara[17]);
          $('#no_spm_lama').val(bendahara[5]);
          $('#norek_bendahara').val(bendahara[9]);
          $('#nama_bank').val(bendahara[10]); 
          $('#npwp_bendahara').val(bendahara[11]);
          $('#jml_spp').val(toRp_WithDecimal(bendahara[14]));
          $('#triwulan').val(bendahara[21]).trigger('change');
          $('#perubahan').val(bendahara[29]).trigger('change');
...
function generate_rinci_spp(no_spp, kodeunit=''){
  ...
  $.ajax({
      ...
      success: function(data){
        ...
        jenispengadaan = form_rinci_spp[49]

        if (jenispengadaan == 2) {
          $('#bendahara').val(form_rinci_spp[50]);
          $('#norek_bendahara').val(form_rinci_spp[27]);
          $('#nama_bank').val(form_rinci_spp[51]); 
          $('#npwp_bendahara').val(form_rinci_spp[52]);
        }else if (jenispengadaan == 0 || jenispengadaan == 1 || jenispengadaan == 'None'){
          $('#bendahara').val(form_rinci_spp[47]);
          $('#norek_bendahara').val(form_rinci_spp[17]);
          $('#nama_bank').val(form_rinci_spp[31]); 
          $('#npwp_bendahara').val(form_rinci_spp[44]);
        }
          // $('#no_spm').val(ambil);
          $('#deskripsi_spp').val(form_rinci_spp[10]);
          $('#status_keperluan').val(form_rinci_spp[11]);
          $('#jml_spm').val(form_rinci_spp[14]);
          $('#jumlah_spm_up').val(toRp_WithDecimal(form_rinci_spp[14]));
          $('#triwulan').val(form_rinci_spp[39]).trigger('change');
          $('#perubahan').val(form_rinci_spp[41]).trigger('change'); 
          $(".cover").hide();
      },
      error:function(err){
        message_ok('error', 'Gagal Ambil Data SPP LS.'); 
      }
  });
}

function generate_rekening(no_spm, kodeunit=''){
...
function simpan_spm(){
  ...
  function elementsToArray(param, type="val"){
   ...
   var rek_potongan_arr=elementsToArray(rek_potongan) ,
    ...
    idbilling_arr = elementsToArray(idbiling),
    ntpn_arr = elementsToArray(ntpn);
 ...
 if($( '#kd_org2').val() == 0){
  ...
  //     $.alertable.alert("Nomor SPM : "+$('#no_spm').val()+" sudah digunakan!"); return false;
      ...
      $.alertable.confirm("Anda yakin akan menyimpan data SPM dengan Nomor : "+$('#no_spm').val().toUpperCase()+" ?").then(function() {       
        $.ajax({
          type: "POST",
          headers: { "X-CSRFToken": csrf_token },
          url: link_simpan_spm,
          data: 
            {
              ...
              'idbiling':JSON.stringify(idbilling_arr),
              'ntpn':JSON.stringify(ntpn_arr),
              'aksi':isSimpan,
            },
            success: function(data){
...
function SimpanSetujuSPM(e){  
  ...
  data.push('skpd='+skpd)
  data.push('kdunit='+$("#kdunit_1").val());

  if(chekSPM == false){
    $.alertable.alert("Nomor SPM Belum ada yang dipilih");return false;
  }else if(data.length == 2 && nosp2d != "-"){
