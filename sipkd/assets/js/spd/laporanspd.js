...
$('#btn_cetak').click(function(){
    ...
    if ($('#kd_org2').val()=='') {
        ...
        $.alertable.alert('Nama Pejabat belum dipilih');
    } else {
        if (var_select=='Kartu Kendali Anggaran' || var_select=='Realisasi SP2D') {
            if ($('#frm_kegiatan').val()=='') {
                $.alertable.alert('Kegiatan belum dipilih');
            } else if (var_select=='Realisasi SP2D' && $('#frm_rekening').val()=='') {
...
function fungsi_modal(ini, asal_modal){
    ...
    if (asal_modal=='pejabat_lap_spd') {
        ...
        $.each(row.find('td:nth-child(4)'), function(){ var_kdsubkegiatan    = $(this).text();});
        $.each(row.find('td:nth-child(5)'), function(){ var_urai    = $(this).text();});
        $('#frm_kegiatan').val(var_kdbidang+'.'+var_kdprogram+'.'+var_kdkegiatan+'.'+var_kdsubkegiatan+' - '+var_urai);

        if($('#jns_laporan').val() == 'Realisasi SP2D'){ $('#frm_rekening').val(''); }
