    	...
    	type: "POST",
          ...
          $('#nip_bendahara').val('');
      		...
      		for (var i = 0; i < JSON.parse(data).length; i++) {
      		  ...
      		  nama_ben = JSON.parse(data)[i]['nama'].replace(/[^a-z0-9\s]/gi, '').replace(/[_\s]/g, '-');
          }
      	}
		});

    set_bendahara(nama_ben);
	}
}

function set_bendahara(nama_ben){
  $("#bendahara > option").each(function() {
    if ($(this).attr('id') == nama_ben) {
	...
	$.ajax({
      ...
      success: function(data){
        ...
        $("#bulan > option").each(function() {
          if ($(this).attr('alt') == rinci.bulan_awal) {
            $(this).prop('selected', true);
  ...
  if ($('#no_spd').val()=='') {
    ...
    const ArrayFromTabel = $("#table_spd tbody tr").map(function(i,row){
      ...
      return {
        ...
        uang: data.eq(4).find("input[type='text']").val()
      }
    }).get();
