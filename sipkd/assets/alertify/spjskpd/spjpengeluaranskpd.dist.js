...
$1('#kd_org2').on('change', function() {
	...
	if (temp === '') kodeskpd = kodeskpd_index = null;
		...
		kodeskpd = { kodeurusan: kodeskpd_index[0], kodesuburusan: kodeskpd_index[1], kodeorganisasi: kodeskpd_index[2], };
	...
	if (kodeskpd) {
		for(var k in kodeskpd) {
			forme(laporan, k, function(input) {
				if (k === 'kodeorganisasi') input.value = window.spjskpd.squote(kodeskpd[k]);
...
$3(xupgux).on('browsed', function(event, entry) {
	...
	if (jenis_bku === 'SP2D') {
		...
		$3.get(xupgux_rincian.dataset.hrefSpjrincian, $3.extend({
			...
			$3(xupgux_rincian).trigger('reload', [result,]);
			$3(xupgux_potongan).trigger('reload', [[],]);
		}).fail(function() {
