...
$('#kd_org2').change(function(){
    clearForm();
    ambilKegiatan(function() {});
    $('#status_keperluan').val('GU NIHIL pada '+$('#kd_org2_urai').val()+' TA. '+DateNowInd().split(' ')[2]+', sesuai dengan bukti terlampir.'); 
});
...
$('#btn_lihat_sp2d').click(function(){
    render_modal('Lihat SP2D', 'sp2d_gunihil', []);
});
...
$('#btn_lihat_spm').click(function(){
    if ($('#kd_org2').val()=='') {
        ...
        render_modal('Lihat SPM', 'spm_sp2d', [$('#kd_org2').val(),'GU_NIHIL']);
      }
});
    ...
    $.ajax({
        ...
        success: function(data){
            if (JSON.parse(data)['hasilnya'].length!=0) {
                ...
                $('#status_keperluan').val('GU NIHIL pada Dinas '+$('#kd_org2_urai').val()+''+DateNowInd()+', sesuai dengan bukti terlampir.'); 
...
function cek_uncek_all_sp2dgunihil(e, chkclass, page){
    ...
    if(e.checked){
        $('#frm_sp2dgunihil input[name=cek_'+page+']').val('1');
        ...
        $('#frm_sp2dgunihil input[name=cek_'+page+']').val('0');
        for (var i = 0; i < pjg_keg; i++) {
