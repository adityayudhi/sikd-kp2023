...
$('#btn_lihat_sp2d').click(function(){
    render_modal('Lihat SP2D', 'sp2d_tu', []);
});
...
$('#btn_lihat_spm').click(function(){
    if ($('#kd_org2').val()=='') {
        ...
        render_modal('Lihat SPM', 'spm_sp2d', [$('#kd_org2').val(),'tu']);
      }
});
...
function ambilKegiatan(){
    if ($('#frm_triwull').val()=='0') {
        ...
        } //end if kd_org tidak kosong
    } //end else kosong triwulan
}

function ambilKegiatan2(callback) {
    array_keg = [];
    if ($('#kd_org2').val()!='') {
        $.ajax({
            type : "POST",
...
function render_tabel_kegiatan(arraynya){
    $('#tabelnya').DataTable( {
        ...
        'createdRow':  function (row, data, index) {
            ...
            $('td', row).eq(3).text(data[2] == 'None' ? toRp_WithDecimal('0'):toRp_WithDecimal(data[2])); //anggaran
            $('td', row).eq(4).text(data[3] == 'None' ? toRp_WithDecimal('0'):toRp_WithDecimal(data[3])); //batas
            $('td', row).eq(5).text(data[4] == 'None' ? toRp_WithDecimal('0'):toRp_WithDecimal(data[4])); //lalu
            $('td', row).eq(6).text(data[5] == 'None' ? toRp_WithDecimal('0'):toRp_WithDecimal(data[5])); //sekarang
            $('td', row).eq(7).text(data[6] == 'None' ? toRp_WithDecimal('0'):toRp_WithDecimal(data[6])); //jumlah
            $('td', row).eq(8).text(data[7] == 'None' ? toRp_WithDecimal('0'):toRp_WithDecimal(data[7])); //sisa
...
function cekTU() {
    $.ajax({
        ...
        success: function(data){
            hasil_cektu = JSON.parse(data)['hasil_cektu'];
            var j = 0;
            for (var i = 0; i < hasil_cektu.length; i++) {
                j = j+1;
            }
            if (j==0) {
                ambilKegiatan2(function() {});
            }else{
                $.alertable.alert('Masih ada '+j+' SP2D TU yang belum di LPJ kan !');
                $('#tabelnya').DataTable().clear().draw();
            }
            $(".cover").hide();
        },
        error: function(){
    ...
    $.ajax({
        ...
        success: function(data){
            if (JSON.parse(data)['hasilnya'].length!=0) {
                ...
                $('#status_keperluan').val(hasilnya['statuskeperluan']); 
...
function ambilSPM(){
    $.ajax({
        ...
        error: function(){
            message_ok('error', 'Data SPM tidak ditemukan!');
            $(".cover").hide();
        }
    });
}
...
function cek_uncek_all_sp2dtu(e, chkclass, page){
    ...
    if(e.checked){
        $('#frm_sp2dtu input[name=cek_'+page+']').val('1');
        ...
        $('#frm_sp2dtu input[name=cek_'+page+']').val('0');
        for (var i = 0; i < pjg_keg; i++) {
...
function simpan_oce_tu(argument) {
