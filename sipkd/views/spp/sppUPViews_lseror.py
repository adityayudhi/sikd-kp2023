...
def deletesppup(request):
    ...
    if gets != '0':
        orgs = gets.split('.')
        skpd = orgs[0]+"."+orgs[1]+"."+orgs[2]+"."+kdunit
    else:
        skpd = '0.0.0.0'
...
def loadspp(request,jenis):
    ...
    if jenis=='gj':
        with connections[tahun_log(request)].cursor() as cursor:
            cursor.execute("SELECT s.kodeunit,s.nospp,s.tglspp as tanggal,(select o.kodeurusan||'.'||o.kodesuburusan||'.'||lpad(o.kodeorganisasi,2,'0')||'-'||o.urai "
        ...
        with connections[tahun_log(request)].cursor() as cursor:
            cursor.execute("SELECT s.kodeunit,s.nospp,s.tglspp as tanggal,(select o.kodeurusan||'.'||o.kodesuburusan||'.'||lpad(o.kodeorganisasi,2,'0')||'.'||o.kodeunit||'-'||o.urai "
                ...
                "and s.kodeorganisasi=%s and s.jenisspp=%s",
                [tahun_log(request),org[0],org[1],org[2],jenis.upper()])
            list_spp = dictfetchall(cursor)
        ...
        if jenis.upper() == 'GU':
            with connections[tahun_log(request)].cursor() as cursor:
                ...
                "and s.kodesuburusan=%s and s.kodeorganisasi=%s and s.jenisspp=%s",
                [tahun_log(request),org[0],org[1],org[2],jenis.upper()])
                list_spp = dictfetchall(cursor)
        else:
            with connections[tahun_log(request)].cursor() as cursor:
...
def loadpihakketiga(request):
    ...
    with connections[tahun_log(request)].cursor() as cursor:
        cursor.execute("SELECT NAMAPERUSAHAAN,BENTUKPERUSAHAAN,ALAMATPERUSAHAAN,NAMAPIMPINANPERUSAHAAN,NPWPPERUSAHAAN,\
            ...
            and jenisspp ='LS' and namaperusahaan <> '' and bentukperusahaan <> '' \
...
def loadkegiatan(request):
    ...
    with connections[tahun_log(request)].cursor() as cursor:
        cursor.execute("SELECT k.KODEUNIT,k.KODEBIDANG,k.KODEPROGRAM,k.KODEKEGIATAN,k.KODESUBKEGIATAN,k.URAI,\
            ...
            and k.KODEORGANISASI=%s and k.kodesubkegiatan<>0 and k.kodesubkeluaran=0 \
            ORDER BY k.KODEUNIT,k.KODEBIDANG,k.KODEPROGRAM,k.KODEKEGIATAN,k.kodesubkegiatan",
            [tahun_log(request),org[0],org[1],org[2]])
...
def loadlpjspp(request,jenis):
    ...
    if jenis=='GU':
        with connections[tahun_log(request)].cursor() as cursor:
            cursor.execute("SELECT s.kodeunit,s.nospj,s.tglspj,s.keperluan,(select (CASE WHEN sum(jumlah) is NULL THEN 0.00 ELSE sum(jumlah) END) from penatausahaan.spj_skpd_rinc_sub1 sr "
                ...
                "(select urai from master.master_organisasi morg WHERE morg.tahun = s.tahun and morg.kodeurusan = s.kodeurusan and morg.kodesuburusan = s.kodesuburusan and morg.kodeorganisasi = s.kodeorganisasi and morg.kodeunit = s.kodeunit) as urai_skpd"
    ...
    objek = []
    for arr in list_lpj:
        objek.append({'urai_skpd':arr['urai_skpd'],'kodeunit':arr['kodeunit'],'nospj':arr['nospj'],'tglspj':tgl_indo(arr['tglspj']),'keperluan':arr['keperluan'],
...
def listpersetujuan(request):
    if 'sipkd_username' in request.session:
        ...
        with connections[tahun_log(request)].cursor() as cursor:
            cursor.execute("SELECT NOSPP,JENISSPP,TGLSPP,KEPERLUAN,dasarspd,kegiatan,kodeunit,JUMLAH,0 as CEK,idspp " 
        ...
        with connections[tahun_log(request)].cursor() as cursor:
            cursor.execute("SELECT NOSPP,JENISSPP,TGLSPP,NOSPM,KEPERLUAN,dasarspd,kegiatan,kodeunit,JUMLAH,tanggal,0 as CEK,idspp " 
                "FROM penatausahaan.fc_lihat_spp_skpd(%s,%s,%s,%s,%s) "
            ...
            if(split_spp[0]=='1'):
                ...
                with connections[tahun_log(request)].cursor() as cursor:
                    cursor.execute("UPDATE penatausahaan.spp set locked=%s where tahun=%s"
                        ...
                        " and kodeunit=%s and NOSPP in (%s)",[locked,tahun,aidi[0],aidi[1],aidi[2],kdunit,nospp])
                if(locked == "Y"):
...
def listafektasi(request,jenis):
    if 'sipkd_username' in request.session:
            ...
            if jenis=='GU' or jenis=='GU_NIHIL':
                ...
                with connections[tahun_log(request)].cursor() as cursor:
                    cursor.execute("SELECT cek,otorisasi,koderekening,uraian,"
                        ...
                        "(CASE WHEN sisa is NULL THEN 0.00 ELSE sisa END) as sisa,ROW_NUMBER () OVER () as nomor "
                        " FROM penatausahaan.fc_view_spp_rincian(%s,%s,%s,lpad(%s,2,'0'),%s,%s,%s,%s,%s,%s,%s,%s) ",
                        [tahun_log(request),aidi[0],aidi[1],aidi[2],kdunit,no_spp.upper(),bidang,program,kegiatan,subkegiatan,tgl_spp,jenis])
        ...
        data = {'list_spp' : ArrayFormater(list_spp),'kodekegiatan':kodekegiatan,'hidden':hidden}
    ...
    if 'sipkd_username' in request.session:
        ...
        with connections[tahun_log(request)].cursor() as cursor:
            cursor.execute("SELECT count(nospp) as nospp FROM penatausahaan.spp WHERE TAHUN=%s and UPPER(NOSPP)=%s "
                ...
                [tahun_log(request),nospp.upper(),aidi[0], aidi[1], aidi[2]])
        ...
        if isppkd=='1':
            with connections[tahun_log(request)].cursor() as cursor:
                cursor.execute("SELECT nospd,to_char(tglspd,'dd/mm/yyyy') as tglspd,sum(jumlahspd) as jumlahspd "
                    "FROM penatausahaan.fc_view_spd(%s,%s,%s,lpad(%s,2,'0'),%s) where KODEPROGRAM=0  "
                    ...
                    if jenis == 'GU':
                        ...
                        with connections[tahun_log(request)].cursor() as cursor:
                            cursor.execute("INSERT INTO PENATAUSAHAAN.SPP (TAHUN,KODEURUSAN,KODESUBURUSAN,KODEORGANISASI,KODEUNIT,NOSPP,TGLSPP,TGLSPP_DRAFT,KODEBIDANG,KODEPROGRAM,KODEKEGIATAN,"\
                                ...
                                " npwp,deskripsipekerjaan,triwulan,perubahan,nospj,waktupelaksanaan,namarekeningbank,uname,jenispengadaan,pemilikrekeningperusahaan)"\
                                " VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                                [tahun_log(request),org[0],org[1],org[2],kd_unit,no_spp.upper(),tanggal_spp,tanggal_spp,'',0,0,status_keperluan,'','NOW()',0,0,total,
                                bendahara,norek_bendahara,jenis,jenisdpa,tgldpa,nomordpa,nama_bank,namaperusahaan,bentukperusahaan,alamatperusahaan,
                                pimpinan,rekperusahaan,nomerkontrak,bank_perusahaan,npwp_perusahaan,npwp_bendahara,status_keperluan,
                                triwulan,perubahan,nospj,pelaksanaan,nm_pemilik_rekening,username(request),jenispengadaan,pemilik_rekening_perusahaan])
                            hasil = ""
                elif aksi == "false":
                    if jenis == 'GU':
                        with connections[tahun_log(request)].cursor() as cursor:
    ...
    if 'sipkd_username' in request.session:
        ...
        with connections[tahun_log(request)].cursor() as cursor:
            cursor.execute("SELECT id,NAMA,NIP,pangkat, jabatan||' ('||nama||')' AS JABATAN  from master.pejabat_skpd  where tahun=%s"
                " and kodeurusan=%s and kodesuburusan=%s and kodeorganisasi=%s and upper(jabatan) like %s and jenissistem=%s",
                [tahun_log(request),aidi[0],aidi[1],aidi[2],'%ANGGARAN%',2])
        ...
        with connections[tahun_log(request)].cursor() as cursor:
            cursor.execute("SELECT id,nama,nip,pangkat, jabatan||' ('||nama||')' AS jabatan  from master.pejabat_skpd where tahun=%s "
                ...
                [tahun_log(request),aidi[0],aidi[1],aidi[2],'%BENDAHARA%',2])
            bendahara = dictfetchall(cursor)         
