...
def listkegiatanbkp(request):
	if 'sipkd_username' in request.session:

		gets = request.POST.get('skpd')         
		nobkp = request.POST.get('nobkp')       
		kondisi = ''
		...
		with connections[tahun_log(request)].cursor() as cursor:
			cursor.execute("SELECT DISTINCT(sr.kodebidang||'.'||sr.kodeorganisasi||'.'||sr.kodeprogram||'.'||sr.kodekegiatan||'.'||sr.kodesubkegiatan) as kegiatan, "
				"sr.kodebidang,sr.kodeprogram,sr.kodekegiatan,sr.kodesubkegiatan,sr.nobkp,sr.kodeorganisasi, sr.kodeunit,"
				"(select urai from master.master_organisasi morg WHERE morg.tahun = sr.tahun and morg.kodeurusan=sr.kodeurusan and morg.kodesuburusan=sr.kodesuburusan and"
				" morg.kodeorganisasi=sr.kodeorganisasi and morg.kodeunit=sr.kodeunit) as urai_skpd,"
				"(select urai from penatausahaan.kegiatan k where k.tahun=sr.tahun and k.kodeurusan=sr.kodeurusan and k.kodesuburusan=sr.kodesuburusan "
				"and k.kodeorganisasi=sr.kodeorganisasi and k.kodeunit=sr.kodeunit and k.kodebidang=sr.kodebidang and k.kodeprogram=sr.kodeprogram and k.kodekegiatan=sr.kodekegiatan and k.kodesubkegiatan=sr.kodesubkegiatan and k.kodesubkeluaran = 0) as uraikegiatan,"
				...
				"and sj.kodeorganisasi=sr.kodeorganisasi and sj.kodeunit=sr.kodeunit and sj.nobkp=sr.nobkp and sj.kodebidang=sr.kodebidang and sj.kodeprogram=sr.kodeprogram "
				...
				"ON(sr.tahun=s.tahun and sr.kodeurusan=s.kodeurusan and sr.kodesuburusan=s.kodesuburusan and sr.kodeorganisasi=s.kodeorganisasi and sr.kodeunit=s.kodeunit and sr.nobkp=s.nobkp) "
				"where sr.tahun=%s and sr.kodeurusan=%s and sr.kodesuburusan=%s and sr.kodeorganisasi=lpad(%s,2,'0') and sr.kodeunit=%s"
				"and s.jenis in ('UP','GU') and s.nobkp =%s order BY sr.nobkp,sr.kodeprogram,sr.kodekegiatan ",
				[tahun_log(request),aidi[0],aidi[1],aidi[2],aidi[3],nobkp])
			list_keg = dictfetchall(cursor) 
		...
		with connections[tahun_log(request)].cursor() as cursor:
			cursor.execute("SELECT (CASE WHEN sum(jumlah) is NULL THEN 0.00 ELSE sum(jumlah) END) AS sp2dup,(select (CASE WHEN sum(jumlah) is NULL THEN 0.00 ELSE sum(jumlah) END) as jumlah from penatausahaan.spj_skpd_rinc_sub1 where tahun=%s "
				"and kodeurusan=%s and kodesuburusan=%s and kodeorganisasi=%s and kodeunit=%s and nospj=%s) FROM penatausahaan.SP2DRINCIAN WHERE TAHUN=%s "
				"and kodeurusan=%s and kodesuburusan=%s and kodeorganisasi=%s and kodeunit=%s and kodeakun=1 and kodekelompok=1 and kodejenis=1 and kodeobjek=3 and koderincianobjek=1",
				[tahun_log(request),aidi[0],aidi[1],aidi[2],aidi[3],nobkp,tahun_log(request),aidi[0],aidi[1],aidi[2],aidi[3]]) 
			list_foot = dictfetchall(cursor)
		...
		data = {'list_keg' : list_keg,'kondisi':kondisi[1:],
			'sp2dup':sp2dup, 'jumlahlpj':jumlahlpj, 'sisaup':sisaup}
		return render(request, 'spp/tabel/tabel_kegiatan_bkp.html', data)
...
def addkegiatanbkp(request):
	if 'sipkd_username' in request.session:
		...
		data = {'add_kegiatan' : ArrayFormater(add_kegiatan)}
		return render(request, 'spp/tabel/tabel_tambah_kegiatan_bkp.html', data)
...
## PILIH REKENING HANYA SATU
# def listlpjupgu_old(request,jenis): 
#     if 'sipkd_username' in request.session:
...
#         return render(request, 'spp/tabel/tabel_rincianlpj_old.html', data)
	...
	if 'sipkd_username' in request.session:
		...
		with connections[tahun_log(request)].cursor() as cursor:
			cursor.execute("SELECT sr.kodeunit, sr.kodesub1 as pilih,sr.uraian,sr.tglbukti, sr.jenisbayar, \
				...
				(select count(tahun) from penatausahaan.spj_skpd_potongan b where b.tahun=sr.tahun and b.kodeurusan=sr.kodeurusan \
				and b.kodesuburusan=sr.kodesuburusan and b.kodeorganisasi=sr.kodeorganisasi and b.kodeunit=sr.kodeunit \
				and b.nospj=sr.nospj and b.nobukti = sr.nobukti) as jml_pot,\
	...
	if 'sipkd_username' in request.session:
		...
		with connections[tahun_log(request)].cursor() as cursor:
			cursor.execute("select NOSP2D,NOSPJ,KEPERLUAN,TGLSPJ FROM penatausahaan.SPJ_SKPD where tahun=%s"
				" and kodeurusan=%s and kodesuburusan=%s and kodeorganisasi=%s and kodeunit = %s and nospj=%s",
				[tahun_log(request),aidi[0],aidi[1],aidi[2],aidi[3],nospj[0]])
...
def deleterincianlpj(request):
	if 'sipkd_username' in request.session:
		...
		if hasil != '':
			with connections[tahun_log(request)].cursor() as cursor:
				cursor.execute("DELETE FROM penatausahaan.spj_skpd_potongan \
					where tahun=%s and kodeurusan=%s and kodesuburusan=%s and kodeorganisasi=%s \
					and kodeunit=%s and nospj=%s and nobukti=%s ",
					[tahun_log(request),skpd[0],skpd[1],skpd[2],skpd[3],nospj,nobukti])
...
## ADD JOEL ======================================================================== 
def lpjupgutu_modal_potongan(request):
	tahun_x = tahun_log(request)
	...
	if request.method == 'POST':
		...
		no_lpj  = data.get("no_lpj")
		tgl_lpj = tgl_short(data.get("tgl_lpj"))
		...
		with connections[tahun_log(request)].cursor() as cursor:
			cursor.execute("DELETE FROM penatausahaan.spj_skpd_potongan WHERE tahun = %s AND kodeurusan = %s \
				AND kodesuburusan = %s AND kodeorganisasi = %s AND kodeunit = %s AND nospj = %s AND nobukti = %s",
				[tahun_x,aidi[0],aidi[1],aidi[2],aidi[3],no_lpj,nobukti])

		## INSERT TABEL POTONGAN ===========================================================================
		for p in range(len(pot_kdrek)):
			if pot_kdrek[p] != "" and nobukti != "":
				with connections[tahun_log(request)].cursor() as cursor:
					cursor.execute("INSERT INTO penatausahaan.spj_skpd_potongan (tahun,\
						kodeurusan,kodesuburusan,kodeorganisasi,kodeunit,nospj,\
						rekeningpotongan,jumlah,jenispotongan,tanggal,nobukti, idbiling,ntpn) \
						VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
						[tahun_x,aidi[0],aidi[1],aidi[2],aidi[3],no_lpj,pot_kdrek[p],toAngkaDec(pot_jumlah[p]),
						pot_jenis[p],tgl_lpj,nobukti, pot_idbiling[p],pot_ntpn[p]])
	...
	# # JIKA REKUWES GET - LOAD MODAL ============================================================================
		...
		eskapde = data.get("s")
		no_lpj  = data.get("n").replace('^','/')
		tgl_lpj = data.get("t").replace('_',' ')
		...
		ArrDT = {'aidirow':aidirow, 'hasil':hasil, 'jnsPot':arrJns, 'nobukti':nobukti, 
			'eskapde':eskapde, 'no_lpj':no_lpj, 'tgl_lpj':tgl_lpj}
		return render(request,'spp/modal/spp_potongan_lihat.html',ArrDT)

def lpgupgutu_kuwitansi(request):
	post    = request.POST
	lapParm = {}       
...
#### ADD PERMOHONAN SPJ ----------------- JOEL 01 DES 22 =======================================================
def listpermohonanlpj(request):
	if 'sipkd_username' in request.session:
		if request.method == 'POST':
			post = request.POST
			aidi = post.get('skpd').split('.')
			datx = post.getlist('dtx[]')
			hasil = 0

			for x in range(len(datx)):
				try:
					with connections[tahun_log(request)].cursor() as cursor:
						cursor.execute("""UPDATE penatausahaan.BKP_SKPD SET locked = %s WHERE TAHUN = %s 
							AND KODEURUSAN = %s AND KODESUBURUSAN = %s AND KODEORGANISASI = lpad(%s,2,'0') 
							AND KODEUNIT = %s AND nopspj = %s """,
							['Y',tahun_log(request),aidi[0],aidi[1],aidi[2],aidi[3],datx[x]])
						hasil = hasil + 1

				except Exception as e:
					print('Wkakakakak....error ya brooh, kie errornya : Duplikasi Primary Key', e)
					hasil = 0
					pass

			if hasil > 0:
				pesan = "success|Permohonan SPJ berhasil disetujui"
			else:
				pesan = "error|Permohonan SPJ gagal disetujui"

			data = {'hasil':hasil, 'pesan':pesan}
			return JsonResponse(data)

		else:
			gets = request.GET
			aidi = gets.get('skpd').split('.')
			aksi = gets.get('aksi')

			with connections[tahun_log(request)].cursor() as cursor:
				cursor.execute("""SELECT a.username,a.nopspj,a.tglpspj,a.keperluan,a.locked,a.jenis,
					...
					and a.kodeunit = %s and a.jenis = %s and a.locked <> 'Y'
					ORDER BY a.tglpspj,a.nopspj DESC""",
					[tahun_log(request),aidi[0],aidi[1],aidi[2],aidi[3],'GU'])
				list_lpj = ArrayFormater(dictfetchall(cursor))

			data = {'list_moh':list_lpj}
			return render(request, 'spp/tabel/tabel_permohonanlpj.html', data)
	else:
		return redirect('sipkd:index')