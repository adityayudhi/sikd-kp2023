	...
	if 'sipkd_username' in request.session:
		...
		with connections[tahun_log(request)].cursor() as cursor:
			cursor.execute("select DISTINCT(sr.kodebidang||'.'||sr.kodeorganisasi||'.'||sr.kodeprogram||'.'||sr.kodekegiatan||'.'||sr.kodesubkegiatan) as kegiatan, "
				...
				"and k.kodeorganisasi=sr.kodeorganisasi and k.kodebidang=sr.kodebidang and k.kodeprogram=sr.kodeprogram and k.kodekegiatan=sr.kodekegiatan and k.kodesubkegiatan=sr.kodesubkegiatan and k.kodesubkeluaran = 0) as uraikegiatan,"
				...
				"and sj.kodeorganisasi=sr.kodeorganisasi and sj.nospj=sr.nospj and sj.kodebidang=sr.kodebidang and sj.kodeprogram=sr.kodeprogram "
				"and sj.kodekegiatan=sr.kodekegiatan) as jumlah from penatausahaan.spj_skpd_rinc sr join penatausahaan.spj_skpd s "
		...
		data = {'list_keg' : list_keg,'kondisi':kondisi[1:],'sp2dup':sp2dup, 'jumlahlpj':jumlahlpj,'sisaup':sisaup}
	...
	if 'sipkd_username' in request.session:
		...
		with connections[tahun_log(request)].cursor() as cursor:
			cursor.execute("select *, ROW_NUMBER () OVER () as nomor, "
				...
				"from penatausahaan.fc_view_kegiatan_lpj (%s,%s,%s,lpad(%s,2,'0'))",
