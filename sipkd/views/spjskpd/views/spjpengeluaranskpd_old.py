...
def data(rq):
	...
	try:
		with querybuilder() as qb:
			qb.execute('SELECT * from pertanggungjawaban.anov_bku_skpd(%s, %s,%s,%s, %s, %s)', cond_1)
			qb.result_many(result['tbody'])
			qb.execute('SELECT * FROM pertanggungjawaban.fc_report_bku_pengeluaran_footer(%s, %s,%s,%s, %s,%s) LIMIT 1', cond_2)
...
def save(rq):
	...
	bkurincian_default = {
		'kodebidang': '', 'kodeprogram': 0, 'kodekegiatan': 0,
	}
