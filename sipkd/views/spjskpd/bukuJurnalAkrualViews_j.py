	...
	if gets != '':
		with connections[tahun_log(request)].cursor() as cursor:
			cursor.execute("SELECT distinct s.noref,s.tanggalbukti,s.jenisjurnal,\
				...
				and kodeorganisasi=s.kodeorganisasi and noref=s.noref ) as debet,\
				...
				and kodeorganisasi=s.kodeorganisasi and noref=s.noref ) as kredit,\
				...
				and s.noref=sr.noref) where s.tahun=%s  and s.kodeurusan=%s and s.kodesuburusan=%s and s.isskpd = 0 \
				and s.kodeorganisasi=%s and s.kodeunit=%s and extract (month from s.tanggalbukti )=%s "+arg+"",
...
# print(request.POST)
# {'organisasi': ['1.1.01.0000'], 'a': ['7'], 'b': ['SPJ'], 'cb_jenis': ['2']
	...
	if b=='SP2D' or b=='SP2D-GJ' or b=='SPJ' or b=='BAYAR-GJ' or b=='PUNGUT-PAJAK' or b=='SETOR-PAJAK' or b=='RK-PPKD' or b=='KOREKSI PENERIMAAN' or b=='KOREKSI PENGELUARAN':
		...
		with connections[tahun_log(request)].cursor() as cursor:
			cursor.execute("select * from penatausahaan.sp2d where tahun=%s and kodeurusan = %s and kodesuburusan = %s and replace(KODEORGANISASI,' ', '') = %s and nosp2d=%s",[tahun_log(request), org[0], org[1], org[2], a])
			hasilSP2D = dictfetchall(cursor)
	elif b=='PUNGUT PENDAPATAN':
		with connections[tahun_log(request)].cursor() as cursor:
			cursor.execute("select * from pertanggungjawaban.skpd_penerimaan where tahun=%s and kodeurusan = %s and kodesuburusan = %s and replace(KODEORGANISASI,' ', '') = %s and isskpd=0 and no_bku=%s and jenis_transaksi='PUNGUT'",[tahun_log(request), org[0], org[1], org[2], a])
			hasilskpdpenerimaan = dictfetchall(cursor)
	elif b=='SETOR PENDAPATAN':
		with connections[tahun_log(request)].cursor() as cursor:
			cursor.execute("select * from pertanggungjawaban.skpd_penerimaan where tahun=%s and kodeurusan = %s and kodesuburusan = %s and replace(KODEORGANISASI,' ', '') = %s and isskpd=0 and no_bku=%s and jenis_transaksi='SETOR'",[tahun_log(request), org[0], org[1], org[2], a])
			hasilskpdsetor = dictfetchall(cursor)

# print(hasilskpdbku)
# [{'tahun': '2021', 'kodeurusan': 1, 'kodesuburusan': 1, 'kodeorganisasi': '01',
# 'kodeunit': '0000', 'no_bku': 7, 'isskpd': 0, 'tgl_bku': datetime.datetime(2021,6, 7, 0, 0), 
# 'urai': 'tes simpan lpj TU', 'jenis_bku': 'SPJ', 'penerimaan': Decimal('0.00'), 'pengeluaran': Decimal('0.00'),
# 'bukti': '00001/LPJ-TU/2021', 'tgl_bukti': datetime.datetime(2021, 5, 20, 0, 0), 'locked': 'T',
# 'jenis_sp2d': 'TU', 'nospj': '', 'status': 0, 'simpananbank': 1, 'uname': 'OJODIHAPUS', 
# 'is_bendahara_pembantu': 'T', 'bendahara_pembantu': None, 'is_pihak_ketiga': 0, 'uname_bantu': None}]
...
def tampilkanTransaksiSKP_akrual_skpd(request):
	...
	with connections[tahun_log(request)].cursor() as cursor:
		cursor.execute("select * from penatausahaan.skp where tahun=%s and kodeurusan = %s and kodesuburusan = %s and replace(KODEORGANISASI,' ', '') = %s and nomor = %s",[tahun_log(request), org[0], org[1], org[2], a])
    	...
    	for i in range(len(array_index)):
    		with connections[tahun_log(request)].cursor() as cursor:
    			cursor.execute("UPDATE akuntansi.AKRUAL_BUKU_JURNAL set posting=%s where tahun=%s"
                    ...
                    " and NOREF = %s",[posting,tahun,org[0],org[1],org[2],array_index[i].split('^')[0]])
    		with connections[tahun_log(request)].cursor() as cursor:
    			cursor.execute("UPDATE akuntansi.AKRUAL_JURNAL_RINCIAN set posting=%s where tahun=%s and KODEURUSAN=%s and KODESUBURUSAN=%s and replace(KODEORGANISASI,' ', '')=lpad(%s,2,'0') and isskpd=0\
                    and NOREF = %s",[posting,tahun,org[0],org[1],org[2],array_index[i].split('^')[0]])
    		if(posting == "1"):
    	...
    	for i in range(len(array_index)):
	        ...
	        with connections[tahun_log(request)].cursor() as cursor:
	            cursor.execute("DELETE FROM akuntansi.AKRUAL_BUKU_JURNAL where tahun=%s"
	                    " and KODEURUSAN=%s and KODESUBURUSAN=%s and replace(KODEORGANISASI,' ', '')=lpad(%s,2,'0') and isskpd=0"
	                    " and NOREF in (%s)",[tahun,org[0],org[1],org[2],array_index[i].split('^')[0]])
	        with connections[tahun_log(request)].cursor() as cursor:
...
def loadlaporanakrual(request):
    ...
    with connections[tahun_log(request)].cursor() as cursor:
        cursor.execute("select id,nama,nip,pangkat, jabatan||' ('||nama||')' AS jabatan from master.pejabat_skpd where tahun=%s"
            " and kodeurusan=%s and kodesuburusan=%s and replace(KODEORGANISASI,' ', '')=lpad(%s,2,'0') and jenissistem=%s order by id ",
					...
					if isSimpan == 'true':
						try:
							with connections[tahun_log(request)].cursor() as cursor:
								...
								tahun_log(request), org[0], org[1], org[2], org[3], 0, noref, nobukti, cb_jenis, uraian, 0, tgl_to_db(tgl), 
						...
						for x in range(len(arr_fix)):
							...
							with connections[tahun_log(request)].cursor() as cursor:
								cursor.execute("INSERT INTO akuntansi.AKRUAL_JURNAL_RINCIAN (TAHUN,\
									...
									x+1, 0, float(arr_fix[x].split('^')[1]),float(arr_fix[x].split('^')[2]), 0])
	...
	if request.method == 'POST':
		...
		with connections[tahun_log(request)].cursor() as cursor:
			cursor.execute('SELECT * \
				FROM akuntansi.fc_dump_jurnal_umum_skpd (%s,%s,%s,%s,%s,%s,%s,%s)',[tahun_log(request),
				org[0],org[1],org[2],'ADMIN',tgl_awal,tgl_akhir,jenis])
			hasilnya = dictfetchall(cursor)
