...
def index(request):
	...
	return render(request, 'kasda/kasda_nota.html', data)
	...
	if jenis.lower() == 'upper':
			...
			with connections[tahun_log(request)].cursor() as cursor:
				...
				if no_bukti != '':
					if cek_noBukti(tahun, no_bukti) >= 1:  # dari support_sipkd
						isSimpan = 0
						pesan = 'NOTA dengan nomor : '+no_bukti+', sudah digunakan !!'
					else:
							...
							try:
								...
								username,locked) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
											[tahun, skpd[0], skpd[1], skpd[2], skpd[3], no_bukukas, no_bukti, deskripsi, '0',
												tgl_transaksi, tgl_bukti, sumdana, 'NOTA', tgl_saiki, uname_x, 'T'])

								xx_nobukas = no_bukukas
								isSimpan = 1
								pesan = 'Nomor NOTA : '+no_bukti+' berhasil disimpan !'

							except IntegrityError as e:
								isSimpan = 0
								pesan = 'Nomor NOTA : '+no_bukti+' sudah ada !'
						else:
							isSimpan = 0
		...
		# AKSI EDIT -> UPDATE ====================================================
			...
			if cek_lock == 'Y':
				...
				isSimpan = 0
				pesan 	 = 'NOTA dengan nomor : '+no_bukti_xx+', sudah digunakan !!'
			else:
				with connections[tahun_log(request)].cursor() as cursor:
					try:
						cursor.execute("UPDATE kasda.kasda_transaksi SET nobukti = %s, tglbukti = %s,\
							...
							tgl_saiki,'NOTA','0',uname_x,skpd[0],skpd[1],skpd[2],skpd[3],tahun,no_bukukas_xx])
						...
						isSimpan = 0
						pesan 	 = 'Nomor NOTA : '+no_bukti_xx+' sudah ada !'
		# SIMPAN DETAIL
		if isSimpan == 1:
			with connections[tahun_log(request)].cursor() as cursor:
		...
		if cek_lock == 'Y':
			...
			if data['aksi'].lower() == 'false':
				with connections[tahun_log(request)].cursor() as cursor:
					...
					isSimpan = 0
					pesan = 'Data NOTA Dengan Nomor Buku Kas: '+no_bukukas_xx+', berhasil dihapus'
