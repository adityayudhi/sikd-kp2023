...
def index(request):
	...
	return render(request, 'kasda/kasda_tahunkemarin.html', data)
					...
					if cek_noBukti(tahun,no_bukti) >= 1: ### dari support_sipkd
						...
						if no_bukukas != '' and data.get('organisasi') != '':
							try:
								...
								tgl_transaksi,tgl_bukti,sumdana,'CONTRAKEMARIN',tgl_saiki,uname_x,'T'])
				...
				with connections[tahun_log(request)].cursor() as cursor:
					try:
						cursor.execute("UPDATE kasda.kasda_transaksi SET nobukukas=%s, nobukti = %s, tglbukti = %s,\
							...
							tgl_saiki,'CONTRAKEMARIN','0',uname_x,skpd[0],skpd[1],skpd[2],skpd[3],tahun,no_bukukas_xx])
