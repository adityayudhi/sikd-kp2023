...
def index(request):
	...
	data = {'dt_sumberdana': dt_sumberdana}
	return render(request, 'kasda/kasda_tahunberjalan.html', data)
...
def simpan(request, jenis):
	...
	if jenis.lower() == 'upper':
		...
		jenissp2d = data.get('jenissp2d')
		dat_kdrek = data.getlist('cut_kdrek')
		dat_JmlTerima = data.getlist('jml_Penerimaan')
		dat_JmlKeluar = data.getlist('jml_Pengeluaran')
		
		if no_bukukas == '':
			hasil = {'pesan':'No Buku Kas Tidak Boleh Kosong', 'issimpan':isSimpan}
			return JsonResponse(hasil)

		try:
			int(no_bukukas)
		except Exception as e:
			hasil = {'pesan':'No Buku Kas Tidak Valid.', 'issimpan':isSimpan}
			return JsonResponse(hasil)

		# VALIDASI TANGGAL
		date_bukti = datetime.strptime(tgl_bukti, '%d-%b-%Y')
		date_transaksi = datetime.strptime(tgl_transaksi, '%d-%b-%Y')
		if date_transaksi < date_bukti:
			pesan = "Tanggal Transaksi tidak boleh lebih kecil dari Tanggal bukti !"
		# AKSI ADD -> INSERT =====================================================
		if aksi.lower() == 'true':
			checked = [2,3]
			for i in range(10):
				if i in checked:
					print(f"nilai {i} ada di dalam list checked")
			with connections[tahun_log(request)].cursor() as cursor:
				no_bukukas = cek_kasda_autoNoKas(request)  # dari support_sipkd
				if no_bukti != '':
					if cek_noBukti(tahun, no_bukti) >= 1:  # dari support_sipkd
						isSimpan = 0
						pesan = 'Pengembalian Tahun Berjalan dengan nomor : '+no_bukti+', sudah digunakan !!'
					else:
						if no_bukukas != '' and data.get('organisasi') != '':
							try:
								...
								username,locked,jenissp2d) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
								[tahun, skpd[0], skpd[1], skpd[2], skpd[3], no_bukukas, no_bukti, deskripsi, '0',
								tgl_transaksi, tgl_bukti, sumdana, 'CONTRA', tgl_saiki, uname_x, 'T', jenissp2d])

								xx_nobukas = no_bukukas
								isSimpan = 1
								pesan = 'Nomor Pengembalian Tahun Berjalan : '+no_bukti+' berhasil disimpan !'

							except IntegrityError as e:
								isSimpan = 0
								pesan = 'Nomor Pengembalian Tahun Berjalan : '+no_bukti+' sudah ada !'
						else:
							isSimpan = 0
		...
		# AKSI EDIT -> UPDATE ====================================================
			...
			elif no_bukti_xx != no_bukti and cek_noBukti(tahun,no_bukti) >= 1:
				isSimpan = 0
				pesan 	 = 'Pengembalian Tahun Berjalan dengan nomor : '+no_bukti_xx+', sudah digunakan !!'
			else:
				with connections[tahun_log(request)].cursor() as cursor:
					try:
						cursor.execute("UPDATE kasda.kasda_transaksi SET nobukti = %s, tglbukti = %s,\
							kodesumberdana = %s, tanggal = %s, deskripsi = %s, lastupdate = %s,\
							jenistransaksi = %s, kdlokasi = %s, username = %s, kodeurusan=%s,\
							kodesuburusan=%s, kodeorganisasi=%s, kodeunit=%s, jenissp2d = %s WHERE tahun = %s\
							AND nobukukas = %s",[ no_bukti,tgl_bukti,sumdana,tgl_transaksi,deskripsi,
							tgl_saiki,'CONTRA','0',uname_x,skpd[0],skpd[1],skpd[2],skpd[3],jenissp2d,tahun,no_bukukas_xx, ])

						xx_nobukas = no_bukukas_xx
						isSimpan = 1
						pesan = 'Perubahan nomor Buku Kas : '+no_bukti+' berhasil disimpan !'

					except IntegrityError as e:
						isSimpan = 0
						pesan 	 = 'Nomor Pengembalian Tahun Berjalan : '+no_bukti_xx+' sudah ada !'
		# SIMPAN DETAIL
		if isSimpan == 1:
			if jenissp2d=='UP' or jenissp2d=='TU' or jenissp2d=='GU':
				with connections[tahun_log(request)].cursor() as cursor:
				...
				rekening_terpilih = data.getlist('daftarRekeningLS')
				# print("========================================================")
				# print(rekening_terpilih)
				# print(len(rekening_terpilih))
				# print(dat_kdrek)
				# print(dat_JmlTerima)
				# print(dat_JmlKeluar)

				if len(rekening_terpilih) > 0:
					with connections[tahun_log(request)].cursor() as cursor:
						# HAPUS TABEL RINCIAN WHERE kasda
						cursor.execute("DELETE FROM kasda.kasda_transaksi_detil WHERE tahun = %s AND nobukukas = %s ",
							[tahun,xx_nobukas])
						...
						for p in range(len(dat_kdrek)):
							if dat_kdrek[p] != "":
								if str(p) in rekening_terpilih:
									...
									KODEAKUN,KODEKELOMPOK,KODEJENIS,KODEOBJEK,KODERINCIANOBJEK,KODESUBRINCIANOBJEK,\
									PENERIMAAN,PENGELUARAN,KODESUBKELUARAN) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
									[tahun,xx_nobukas,skpd[0],skpd[1],skpd[2],skpd[3],
									obj1[0]+'.'+obj1[1],obj1[3],obj1[4]+'.'+obj1[5],obj1[6],
									obj2[0],obj2[1],obj2[2],obj2[3],obj2[4],obj2[5],rpin,rpot,0])
		...
		if cek_lock == 'Y':
			...
			if data['aksi'].lower() == 'false':
				with connections[tahun_log(request)].cursor() as cursor:
					...
					isSimpan = 0
					pesan = 'Data Pengembalian Tahun Berjalan Dengan Nomor Buku Kas: '+no_bukukas_xx+', berhasil dihapus'
