...
def index(request):
	...
	return render(request, 'kasda/kasda_saldo.html', data)
...
def simpan(request,jenis):
	...
	if jenis.lower() == 'upper': 
			...
			with connections[tahun_log(request)].cursor() as cursor:
				...
				if no_bukti != '':
					if cek_noBukti(tahun,no_bukti) >= 1: ### dari support_sipkd
						isSimpan = 0
						pesan 	 = 'Bukti dengan nomor : '+no_bukti+', sudah digunakan !!'
					else:
						if no_bukukas != '' and data.get('organisasi') != '':
							try:
								...
								tgl_transaksi,tgl_bukti,sumdana,'AWAL',tgl_saiki,uname_x,'T'])
								
								xx_nobukas = no_bukukas
								isSimpan = 1
								pesan 	 = 'Nomor Bukti : '+no_bukti+' berhasil disimpan !'

							except IntegrityError as e:
								isSimpan = 0
								pesan 	 = 'Nomor Bukti : '+no_bukti+' sudah ada !'
						else:
							isSimpan = 0
		...
		# AKSI EDIT -> UPDATE ====================================================
			...
			elif no_bukti_xx != no_bukti and cek_noBukti(tahun,no_bukti_xx) >= 1:
				isSimpan = 0
				pesan 	 = 'Bukti dengan nomor : '+no_bukti_xx+', sudah digunakan !!'
			else:
				with connections[tahun_log(request)].cursor() as cursor:
					try:
						cursor.execute("UPDATE kasda.kasda_transaksi SET nobukti = %s, tglbukti = %s,\
							...
							tgl_saiki,'AWAL','0',uname_x,skpd[0],skpd[1],skpd[2],skpd[3],tahun,no_bukukas_xx])

						xx_nobukas = no_bukukas_xx
						isSimpan = 1
						pesan = 'Perubahan nomor Buku Kas : '+no_bukti_xx+' berhasil disimpan !'

					except IntegrityError as e:
						isSimpan = 0
						pesan 	 = 'Nomor Bukti : '+no_bukti_xx+' sudah ada !'
		...
		if cek_lock == 'Y':
			...
			if data['aksi'].lower() == 'false':
				with connections[tahun_log(request)].cursor() as cursor:
					...
					isSimpan = 0
					pesan = 'Data Bukti Dengan Nomor Buku Kas: '+no_bukukas_xx+', berhasil dihapus'
