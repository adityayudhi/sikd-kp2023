...
def get_skp_skr_skpd(request):
	tahun_x = tahun_log(request)
	akses_x = hakakses(request).upper()
	sipkd_perubahan = perubahan(request)

	arrDT = {'akses':akses_x, 'perubahan':str(sipkd_perubahan), 'jenis':'all'}
	return render(request,'spjppkd/skp_skr_skpd.html',arrDT)

def skpskr_skpd_get_tbl_awal(request):
	tahun_x = tahun_log(request)
	data  	= request.POST
	...
	if len(data.get("skpd"))>0:
		...
		with connections[tahun_log(request)].cursor() as cursor:
			cursor.execute("SELECT 0 as CEK, row_number() over(order by tanggal desc) as nomer, \
				...
				and a.kodesuburusan = %s and a.kodeorganisasi = %s and a.ISSKPD = 0 "+argtex,
				[tahun_x,int(skpd[0]),int(skpd[1]),str(skpd[2])])
			arrTBL = ArrayFormater(dictfetchall(cursor))

	arrDT = {'arrTabel':arrTBL}
	return render(request,'spjppkd/tabel/skp_skr_skpd.html',arrDT)

def skpskr_skpd_modal_in(request):
	tahun_x  = tahun_log(request)
	...
	if aksi == "false":
		...
		with connections[tahun_log(request)].cursor() as cursor:
			cursor.execute("SELECT row_number() over() as nomer, urai as uraian,jumlah,\
				...
				AND kodeurusan = %s AND kodesuburusan = %s AND kodeorganisasi = %s AND a.isskpd = 0",
	...
	return render(request,'spjppkd/modal/skp_skr_skpd_input.html',arrDT)

def skpskr_skpd_modal_edit(request):
	tahun_x = tahun_log(request)
	data  	= request.POST
	skpd 	= data.get('skpd').split('.')
	...
	with connections[tahun_log(request)].cursor() as cursor:
		cursor.execute("SELECT *,case when UPPER(jenis)='SKP' then 0 else 1 end as pilih \
			...
			AND kodeurusan = %s AND kodesuburusan = %s AND kodeorganisasi = %s AND isskpd = 0 limit 1",
			[tahun_x,nomor,skpd[0],skpd[1],skpd[2]])
		hasil = ArrayFormater(dictfetchall(cursor))

	return JsonResponse(hasil[0])

def skpskr_skpd_modal_rek(request):
	data 	= request.GET
	tahun_x = tahun_log(request)
	aidirow = data.get("i")
	...
	with connections[tahun_log(request)].cursor() as cursor:
		cursor.execute("SELECT KODEAKUN||'.'||KODEKELOMPOK||'.'||KODEJENIS||'.'||LPAD(KODEOBJEK::text,2,'0')||'.'||LPAD(KODERINCIANOBJEK::text,2,'0') AS KODEREKENING, URAI,0 as JUMLAH \
			FROM master.master_rekening WHERE TAHUN = %s AND KODEAKUN = 4 AND KODEKELOMPOK <> 0 \
	...
	return render(request,'spjppkd/modal/skp_skr_skpd_rekening.html',ArrDT)

def skpskr_skpd_set_simpan(request, jenis):
	isSimpan = 0
	data 	 = request.POST
	...
	if jenis.lower() == 'upper': # JENIS UNTUK ADD dan EDIT ============================
		...
		if aksi.lower() == 'true': # AKSI ADD -> INSERT ==========================================================
			...
			arrayDT = [tglSKP,nomorSKP,uraianSKP,jenisSKP,wajibBYR,alamatSKP,noPk_SKP,masa_SKP,tglTEMPO,tahun,skpd[0],skpd[1],skpd[2],nomorSKP_X]
			...
			if (nomorSKP != nomorSKP_X):
				if ck_nomorSKP(tahun, nomorSKP) >= 1:
					...
					isSimpan = update_skpskr_skpd(arrayDT)
		...
		if isSimpan == 1:
			with connections[tahun_log(request)].cursor() as cursor:
				...
				cursor.execute("DELETE FROM penatausahaan.SKP_RINCIAN WHERE tahun = %s \
					AND kodeurusan = %s AND kodesuburusan = %s AND kodeorganisasi = %s AND nomor = %s AND isskpd = 0",
					[tahun,skpd[0],skpd[1],skpd[2],nomorSKP])
...
def update_skpskr_skpd(arrayDT):
	try:
		with connections[tahun_log(request)].cursor() as cursor:
...
def skpskr_skpd_frm_lap(request):
	tahun = tahun_log(request)
	...
	if request.method == 'POST':
		...
		with connections[tahun_log(request)].cursor() as cursor:
			cursor.execute("SELECT id,nama,nip,pangkat,jabatan||' ('||nama||')' AS jabatan \
				...
				AND kodeorganisasi = %s AND jenissistem = 2 ",[tahun,skpd[0],skpd[1],skpd[2]])
			hasil = ArrayFormater(dictfetchall(cursor))
		
		arrDT = { 'ls_data':hasil, 'skpd':data.get('id'), 'masa':masa, 'nomor':nomor, 'jenis':jenis}

		return render(request,'spjppkd/laporan/skp_skr_skpd.html',arrDT)
