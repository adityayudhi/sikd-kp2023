...
def get_koreak_load(request):
	...
	with connections[tahun_log(request)].cursor() as cursor:
		cursor.execute("SELECT kodeakun, urai, kodeakun||'-'||urai AS urairekening FROM master.master_rekening \
			WHERE tahun = %s AND kodeakun IN (2,4,5,6) AND kodekelompok = 0 \
...
def koreak_get_tbl_awal(request):
	...
	if (jns_akun == 1): where = "where a_kodeakun=1 and a_kodekelompok<>3"
		...
		where = "where a_kodeakun=5 and a_kodejenis=1"
		gdlo  = 'Rekening Beban LO PMD 64'
	elif (jns_belanja == 2 and jns_akun == 5):
		where = "where a_kodeakun=5 and a_kodekelompok=2 and a_kodejenis=2"
		gdlo  = 'Rekening Beban LO PMD 64'
	elif (jns_belanja == 3 and jns_akun == 5):
		where = "where a_kodeakun=5 and a_kodekelompok=2 and a_kodejenis=3"
		gdlo  = 'Rekening Aset PMD 64'
	elif (jns_belanja == 4 and jns_akun == 5):
		where = "where a_kodeakun=5 and a_kodekelompok=1 and a_kodejenis<>1"
		gdlo  = 'Rekening Beban LO PMD 64'
	...
	with connections[tahun_log(request)].cursor() as cursor:
		cursor.execute("SELECT * FROM akuntansi.fc_view_konversi_akrual(%s,%s) "+where+" \
			order by a_kodeakun,a_kodekelompok,a_kodejenis,a_kodeobjek,a_koderincianobjek",
			[tahun_x,jns_akun])
...
def koreak_modal_in(request):
	...
	ur_c = kd_c_0 = kd_c_1 = kd_c_2 = kd_c_3 = kd_c_4 = ''
	...
	arrDT = {'hidden':hidden, 'lblkorolari':lblkorolari,
		...
		'edkdRincObjek_C':kd_c_4,'edUraian_C':ur_c}
...
def mdl_akrual_rekening(request,jenis):
	...
	with connections[tahun_log(request)].cursor() as cursor:
		cursor.execute("SELECT KODEAKUN||'.'||KODEKELOMPOK||'.'||KODEJENIS||'.'||lpad(KODEOBJEK::TEXT,2,'0')||'.'||lpad(KODERINCIANOBJEK::TEXT,2,'0') as KODEREKENING,URAI \
 			FROM "+dbase+" WHERE TAHUN = %s AND KODERINCIANOBJEK <> 0 "+pilter+" ORDER BY KODEAKUN,KODEKELOMPOK,KODEJENIS,KODEOBJEK,KODERINCIANOBJEK",[tahun])
		arrTBL = dictfetchall(cursor)
...
def koreak_set_simpan(request,jenis):
	...
	else: edkdAkun_C = edkdKelompok_C = edkdJenis_C = edkdObjek_C = edkdRincObjek_C = ""
			...
			if edkdAkun_B != "" or edkdAkun_C != "":
				with connections[tahun_log(request)].cursor() as cursor:
					cursor.execute("INSERT INTO akuntansi.konversi_akrual (tahun,kodeakun,kodekelompok,kodejenis,\
						...
						lob_kodeakun,lob_kodekelompok,lob_kodejenis,lob_kodeobjek,lob_koderincianobjek)\
						VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
						[tahun,edkdAkun_A,edkdKelompok_A,edkdJenis_A,edkdObjek_A,edkdRincObjek_A,
						edkdAkun_B,edkdKelompok_B,edkdJenis_B,edkdObjek_B,edkdRincObjek_B,
						edkdAkun_C,edkdKelompok_C,edkdJenis_C,edkdObjek_C,edkdRincObjek_C])
				isSimpan = 1
