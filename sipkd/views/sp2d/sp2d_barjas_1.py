...
def sp2d_barjas_cari_spm_sp2d(request):
	...
	with connections[tahun_log(request)].cursor() as cursor:
		if asal == 'src_spm':
			cursor.execute("SELECT row_number() over(ORDER BY s.tanggal DESC) as nomor,'' as nosp2d,s.nospm,s.tanggal,\
				...
				and sr.kodesuburusan=s.kodesuburusan and sr.kodeorganisasi=s.kodeorganisasi and sr.nospm=s.nospm limit 1 ) as kodebidang,\
				(select kodeprogram from penatausahaan.spmrincian sr where sr.tahun=s.tahun and sr.kodeurusan=s.kodeurusan\
				and sr.kodesuburusan=s.kodesuburusan and sr.kodeorganisasi=s.kodeorganisasi and sr.nospm=s.nospm limit 1 ) as kodeprogram,\
				(select kodekegiatan from penatausahaan.spmrincian sr where sr.tahun=s.tahun and sr.kodeurusan=s.kodeurusan\
				and sr.kodesuburusan=s.kodesuburusan and sr.kodeorganisasi=s.kodeorganisasi and sr.nospm=s.nospm limit 1 ) as kodekegiatan,\
				s.kodeurusan,s.kodesuburusan,s.kodeorganisasi \
				from penatausahaan.spm s where s.tahun= %s and s.kodeurusan = %s \
				and s.kodesuburusan = %s and s.kodeorganisasi = %s and s.jenisspm = %s and  locked = 'Y' \
				and (nospm not in (select nospm from penatausahaan.sp2d where tahun = %s \
				and kodeurusan = %s and kodesuburusan = %s \
				and kodeorganisasi = %s and jenissp2d = %s))",
			...
			cursor.execute("SELECT row_number() over(ORDER BY s.tanggal DESC) as nomor,s.nosp2d,s.nospm,s.tanggal,s.statuskeperluan as keperluan,\
				...
				and sr.kodesuburusan=s.kodesuburusan and sr.kodeorganisasi=s.kodeorganisasi and  sr.nosp2d=s.nosp2d limit 1 ) as kodebidang,\
				(select kodeprogram from penatausahaan.sp2drincian sr where sr.tahun=s.tahun and sr.kodeurusan=s.kodeurusan\
				and sr.kodesuburusan=s.kodesuburusan and sr.kodeorganisasi=s.kodeorganisasi and  sr.nosp2d=s.nosp2d limit 1 ) as kodeprogram,\
				(select kodekegiatan from penatausahaan.sp2drincian sr where sr.tahun=s.tahun and sr.kodeurusan=s.kodeurusan\
				and sr.kodesuburusan=s.kodesuburusan and sr.kodeorganisasi=s.kodeorganisasi and  sr.nosp2d=s.nosp2d limit 1 ) as kodekegiatan,\
				s.kodeurusan,s.kodesuburusan,s.kodeorganisasi\
				FROM penatausahaan.sp2d s  where s.tahun = %s and kodeurusan = %s and kodesuburusan = %s \
				and kodeorganisasi = %s and s.jenissp2d = %s",[tahun_x,kdur,kdsub,kdorg,jnsSPM])
			hasil = dictfetchall(cursor)

	for xs in hasil:
		with connections[tahun_log(request)].cursor() as cursor:
			cursor.execute("SELECT urai FROM penatausahaan.kegiatan WHERE tahun = %s AND kodeurusan = %s \
				AND kodesuburusan = %s AND kodeorganisasi = %s AND kodebidang = %s \
				AND kodeprogram = %s AND kodekegiatan = %s",
				[tahun_x,kdur,kdsub,kdorg,xs['kodebidang'],xs['kodeprogram'],xs['kodekegiatan']])
			x_urai = dictfetchall(cursor)

		if len(x_urai) >= 1: uraian_x = x_urai[0]['urai']
		else: uraian_x = ''

		kegiatan = xs['kodebidang']+"|"+str(xs['kodeprogram'])+"|"+str(xs['kodekegiatan'])
		arrRinc.append({'nomor':xs['nomor'],'nosp2d':xs['nosp2d'],'nospm':xs['nospm'],'tanggal':tgl_indo(xs['tanggal']),
			'organisasi':xs['organisasi'],'keperluan':xs['keperluan'],'jumlah':xs['jumlah'],'kegiatan':kegiatan,
			'uraian':uraian_x })
...
def sp2d_barjas(request):
	...
	return render(request,'sp2d/sp2d_barjas.html',arrData)

def sp2d_barjas_tabel(request):
	data 	= request.POST
		...
		with connections[tahun_log(request)].cursor() as cursor:
			if(asal == 'SP2D'):
				cursor.execute("SELECT * FROM penatausahaan.fc_view_sp2d_rincian(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
					[tahun, skpd[0],skpd[1],skpd[2],nosp2d,kd_bid,kd_prog,kd_keg,tanggal,'LS'])
			elif(asal == 'SPM'):
				cursor.execute("SELECT * FROM penatausahaan.fc_view_spm_rincian_to_sp2d_rincian(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
					[tahun, skpd[0],skpd[1],skpd[2],nospm,kd_bid,kd_prog,kd_keg,tanggal,'LS'])
			hasil = dictfetchall(cursor)
...
def sp2d_barjas_sumberdana(request):
	data 	= request.POST
...
def sp2d_barjas_rekening(request):
	data  = request.POST
	selek = ''
...
def sp2d_barjas_ambil_spm(request):
	data 	= request.POST
	tahun 	= tahun_log(request)
...
def sp2d_barjas_ambil_sp2d(request):
	data 	= request.POST
	tahun 	= tahun_log(request)
...
def sp2d_barjas_potongan(request):
	data 	= request.POST
...
def sp2d_barjas_mdl_cut(request):
	data 	= request.GET
	tahun_x = tahun_log(request)
	aidirow = data.get("i")
...
def sp2d_barjas_simpan(request, jenis):
	isSimpan = 0
	data 	 = request.POST
