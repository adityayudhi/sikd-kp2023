...
def tu(request):
	...
	return render(request, 'sp2d/tu_kepmen.html', arrData)

def sp2d_tu_kepmen_cari_spm_sp2d(request):
	tahun_x = tahun_log(request)
...
def sp2d_tu_kepmen_tabel(request):
	data 	= request.POST
	...
	tot_sekarang = kd_prog = kd_subkeg = 0.00
	...
	if data.get('kegiatan') != "":
		...
		kd_bid = data.get('kodebidang')
		kd_prog = data.get('kodeprogram')
		kd_keg = data.get('kodekegiatan')
		kd_subkeg = data.get('kodesubkegiatan')

	if data.get('skpd') != '':
		with connections[tahun_log(request)].cursor() as cursor:
			if(asal == 'SP2D'):
				cursor.execute("SELECT * FROM penatausahaan.fc_view_sp2d_rincian(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
					[tahun,skpd[0],skpd[1],skpd[2],kodeunit,nosp2d,kd_bid,kd_prog,kd_keg,kd_subkeg,tanggal,'TU'])

			elif(asal == 'SPM'):
				cursor.execute("""SELECT kodebidang, kodeprogram, kodekegiatan, kodesubkegiatan FROM penatausahaan.spmrincian WHERE tahun = %s and kodeurusan = %s and kodesuburusan = %s 
					and kodeorganisasi = %s and kodeunit = %s and nospm = %s group by tahun, kodeurusan, kodesuburusan, kodeorganisasi, kodeunit, nospm, kodebidang, kodeprogram, kodekegiatan, kodesubkegiatan""", 
					[tahun, skpd[0], skpd[1], skpd[2], kodeunit, nospm])

				hasil_spm = cursor.fetchone()
				kd_bid = hasil_spm[0]
				kd_prog = hasil_spm[1]
				kd_keg = hasil_spm[2]
				kd_subkeg = hasil_spm[3]

				cursor.execute("SELECT * FROM penatausahaan.fc_view_spm_rincian_to_sp2d_rincian(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
					[tahun,skpd[0],skpd[1],skpd[2],kodeunit,nospm,kd_bid,kd_prog,kd_keg,kd_subkeg,tanggal,'TU'])
			hasil = dictfetchall(cursor)

		for i in range(len(hasil)):
			hasil[i].update({'nomer':1+i})

			tot_sekarang += float(hasil[i]['sekarang'])
...
def sp2d_tu_kepmen_sumberdana(request):
	data 	= request.POST
	...
	with connections[tahun_log(request)].cursor() as cursor:
		cursor.execute("SELECT distinct kodesumberdana,sumberdana "\
			...
			[tahun,skpd[0],skpd[1],skpd[2],skpd[3],nosp2d,kd_bid,kd_prog,kd_keg,kd_subkeg,tanggal,'TU'])
		hasil = dictfetchall(cursor)
...
def sp2d_tu_kepmen_rekening(request):
	data  = request.POST
	selek = ''
...
def sp2d_tu_kepmen_ambil_spm(request):
	data 	= request.POST
	tahun 	= tahun_log(request)
	nospm 	= data.get('nospm').upper() 
	skpd 	= data.get('skpd').split('.')
	kdunit 	= data.get('kodeunit')
	...
	with connections[tahun_log(request)].cursor() as cursor:
		cursor.execute("select nospm, tanggal, informasi, norekbank, bank, npwp, namayangberhak, triwulan\
			...
			[tahun,skpd[0],skpd[1],skpd[2],skpd[3],nospm])
		hasil = dictfetchall(cursor)
...
def sp2d_tu_kepmen_ambil_sp2d(request):
	data 	= request.POST
	tahun 	= tahun_log(request)
...
def sp2d_tu_kepmen_potongan(request):
	data 	= request.POST
...
def sp2d_tu_kepmen_mdl_cut(request):
	data 	= request.GET
	tahun_x = tahun_log(request)
	aidirow = data.get("i")
...
def sp2d_tu_kepmen_frm_lap(request):
...
def sp2d_tu_kepmen_simpan(request, jenis):
	isSimpan = 0
	data 	 = request.POST
					...
					if ck_no_sp2d(tahun, nosp2d) >= 1:
						...
						if norekbankasal != '' and data.get('bendahara') != '':
							try:
								...
								tgl_saiki,'TU',sumdana,data.get('status_keperluan'),
