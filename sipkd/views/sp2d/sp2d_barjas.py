	...
	with connections[tahun_log(request)].cursor() as cursor:
		if asal == 'src_spm':
			cursor.execute("SELECT row_number() over(ORDER BY s.tanggal DESC) as nomor,'' as nosp2d,s.nospm,s.tanggal,\
				...
				and s.kodeorganisasi = %s and kodeunit = %s and s.jenisspm = %s and  locked = 'Y' \
			...
			# 	FROM penatausahaan.sp2d s  where s.tahun = %s and s.jenissp2d = %s limit 1000 ",
			# 	[tahun_x,jnsSPM])			
			# hasil = dictfetchall(cursor)
		...
		with connections[tahun_log(request)].cursor() as cursor:
			if(asal == 'SP2D'):
				cursor.execute("SELECT * FROM penatausahaan.fc_view_sp2d_rincian(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
					[tahun,skpd[0],skpd[1],skpd[2],kodeunit,nosp2d,kd_bid,kd_prog,kd_keg,kd_subkeg,tanggal,'LS'])
			elif(asal == 'SPM'):
...
def sp2d_barjas_sumberdana(request):
	...
	with connections[tahun_log(request)].cursor() as cursor:
		cursor.execute("SELECT distinct kodesumberdana,sumberdana "\
			...
			[tahun,skpd[0],skpd[1],skpd[2],skpd[3],nosp2d,kd_bid,kd_prog,kd_keg,kd_subkeg,tanggal,'LS'])
		hasil = dictfetchall(cursor)
...
def sp2d_barjas_ambil_spm(request):
	...
	with connections[tahun_log(request)].cursor() as cursor:
		cursor.execute("select nospm, tanggal, statuskeperluan, norekbank, bank, npwp, namayangberhak, triwulan\
	...
	nmrspm = tanggal = info = norek = bank = npwp = berhak = triwul = ""
...
# JENIS UNTUK ADD dan EDIT ============================
	if jenis.lower() == 'upper': 
		...
		pot_biling 		= data.getlist('idbiling')
		pot_ntpn 		= data.getlist('ntpn')
		jenis_potongan 	= data.getlist('jns_cut')
		kegiatan 		= data.get('kegiatan').split('.')
		kd_bid 			= str(kegiatan[0]+"."+kegiatan[1])
		kd_prog 		= kegiatan[2]
		kd_keg 			= kegiatan[3]
...
# AKSI EDIT -> UPDATE ====================================================
			...
			cek_lock = cek_isLocked(tahun,nosp2d_x,skpd[0],skpd[1],skpd[2],skpd[3])
				...
				for p in range(len(pot_kdrek)):
					if pot_kdrek[p] != "":
						cursor.execute("INSERT INTO penatausahaan.SP2DPOTONGAN (TAHUN,KODEURUSAN,KODESUBURUSAN,\
							KODEORGANISASI,KODEUNIT,NOSP2D,TANGGAL,REKENINGPOTONGAN,JUMLAH,JENISPOTONGAN, IDBILING, ntpn) \
							VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",[tahun,skpd[0],skpd[1],skpd[2],kodeunit,
							nosp2d,tglsp2d,pot_kdrek[p],toAngkaDec(pot_jumlah[p]),pot_jenis[p],pot_biling[p],pot_ntpn[p]])
